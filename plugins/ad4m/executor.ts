...
export async function isExecutorRunning(
  ...
  // MCP probe: the server returns HTTP 422 for `tools/list` without proper
  // session initialization, but any readable response (including 422) means
  // the server IS running. So we don't filter by `res.ok` here.
  ...
  // HTTP/REST probe: validate the JSON payload to positively identify the
  // listener as the AD4M executor (not just any HTTP 200 on port 12000).
  const restProbe = (
    url: string,
    init: RequestInit,

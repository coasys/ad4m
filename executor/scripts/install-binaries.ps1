...
if ($HoloPath -ne $null) {
  if (Test-Path $HoloPath) {
    ...
    if($Version -notlike '*0.1.0-beta-rc.1*') {
      cargo install --locked holochain --git https://github.com/holochain/holochain.git --tag holochain-0.1.0-beta-rc.1 --force --locked
    }
  }
} else {
  cargo install --locked holochain --git https://github.com/holochain/holochain.git --tag holochain-0.1.0-beta-rc.1 --force --locked
...
if ($HcPath -ne $null) {
  if (Test-Path $HcPath) {
    ...
    if($Version -notlike '*0.1.0-beta-rc.1*') {
      cargo install holochain_cli --version 0.1.0-beta-rc.1 --force --locked
    }
  }
} else {
  cargo install holochain_cli --version 0.1.0-beta-rc.1 --force --locked

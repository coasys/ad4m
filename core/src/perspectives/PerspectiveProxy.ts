...
import { collectionAdderToName, collectionRemoverToName, collectionSetterToName, collectionToAdderName, collectionToRemoverName, collectionToSetterName, propertyNameToSetterName } from "../model/util";
...
class Subject {
    ...
    async init() {
        ...
        for(let p of properties) {
            await this.#perspective.infer(`subject_class("${this.#subjectClassName}", C), property_resolve(C, "${p}")`)
        ...
        for(let setter of (setters ? setters : [])) {
            if(setter) {
                ...
                this[propertyNameToSetterName(property)] = async (value: any) => {
                    if(resolveLanguage) { value = await this.#perspective.createExpression(value, resolveLanguage) }

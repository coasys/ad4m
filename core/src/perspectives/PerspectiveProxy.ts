    ...
    try {
      ...
      this.#initTimeoutId = setTimeout(() => {
        ...
        this.subscribe().catch((error) => {
          console.error(
            "Error during subscription retry after timeout:",
            error,
          );
        });
      }, 30000);
    ...
    const keepaliveLoop = async () => {
      ...
      try {
        ...
        try {
          ...
          console.error(
            "Error during resubscription from keepalive:",
            resubscribeError,
          );
...
export class PerspectiveProxy {
  ...
  async getSingleTarget(query: LinkQuery): Promise<string | void> {
    ...
    if (foundLinks.length) return foundLinks[0].data.target;
    else return null;
  }
  ...
  async getSdna(): Promise<string[]> {
    ...
    for (const predicate of sdnaPredicates) {
      ...
      const nameLinks = await this.get(
        new LinkQuery({
          source: "ad4m://self",
          predicate: predicate,
        }),
      ...
      for (const nameLink of nameLinks) {
        ...
        const sdnaLinks = await this.get(
          new LinkQuery({
            source: nameLiteral,
            predicate: "ad4m://sdna",
          }),
  ...
  async addShacl(
    name: string,
    shape: import("../shacl/SHACLShape").SHACLShape,
  ): Promise<void> {
  ...
  async getShacl(
    name: string,
  ): Promise<import("../shacl/SHACLShape").SHACLShape | null> {
    // Find the shape URI from the name mapping
    const nameMapping = Literal.fromUrl(`literal://string:shacl://${name}`);
    const shapeUriLinks = await this.get(
      new LinkQuery({
        source: nameMapping.toUrl(),
        predicate: "ad4m://shacl_shape_uri",
      }),
    ...
    const shapeTypeLinks = await this.get(
      new LinkQuery({
        source: shapeUri,
        predicate: "rdf://type",
      }),
    ...
    const targetClassLinks = await this.get(
      new LinkQuery({
        source: shapeUri,
    ...
    const constructorLinks = await this.get(
      new LinkQuery({
        source: shapeUri,
    ...
    const destructorLinks = await this.get(
      new LinkQuery({
        source: shapeUri,
   ...
   */
  async getAllShacl(): Promise<
    Array<{ name: string; shape: import("../shacl/SHACLShape").SHACLShape }>
  > {
    const nameLinks = await this.get(
      new LinkQuery({
        source: "ad4m://self",
  ...
  async addFlow(
    name: string,
    flow: import("../shacl/SHACLFlow").SHACLFlow,
  ): Promise<void> {
  ...
  async getFlow(
    name: string,
  ): Promise<import("../shacl/SHACLFlow").SHACLFlow | null> {
    const flowNameLiteral = Literal.from(name).toUrl();

    // Find flow URI from name mapping
    const flowUriLinks = await this.get(
      new LinkQuery({
        source: flowNameLiteral,
        predicate: "ad4m://flow_uri",
      }),
    ...
    const flowLinks = allLinks
      .map((l) => l.data)
      .filter(
        (l) => l.source === flowUri || l.source.startsWith(alternatePrefix),
      );
  ...
  async stringOrTemplateObjectToSubjectClassName<T>(
    subjectClass: T,
  ): Promise<string> {
    if (typeof subjectClass === "string") return subjectClass;
    else {
      let subjectClasses = await this.subjectClassesByTemplate(
        subjectClass as object,
      );
      if (subjectClasses[0]) {
  ...
  async createSubject<T, B extends string | undefined = undefined>(
    ...
    return this.getSubjectProxy(exprAddr, subjectClass) as Promise<
      B extends undefined ? T : string
    >;
  }
   ...
   */
  private async getActionsFromSHACL(
    className: string,
    predicate: string,
  ): Promise<any[] | null> {
      ...
      if (shapePattern.test(link.data.source)) {
        ...
        if (link.data.target.startsWith(prefix)) {
          ...
          try {
            ...
            console.warn(
              `Failed to parse SHACL actions JSON for ${className}:`,
              e,
            );
            return null;
          }
  ...
  async removeSubject<T>(subjectClass: T, exprAddr: string, batchId?: string) {
    let className =
      await this.stringOrTemplateObjectToSubjectClassName(subjectClass);

    // Get destructor actions from SHACL links (Prolog-free)
    let actions = await this.getActionsFromSHACL(
      className,
      "ad4m://destructor",
    );
  ...
  async isSubjectInstance<T>(
    ...
    if (!metadata) {
      console.warn(
        `isSubjectInstance: No SHACL metadata found for class ${className}`,
      );
      return false;
    }
    ...
    if (metadata.requiredTriples.length === 0) {
      ...
      const countValue =
        typeof count === "object" && count?.Int !== undefined
          ? count.Int
          : count;
      return countValue > 0;
    }
  ...
  async getSubjectClassMetadataFromSDNA(className: string): Promise<{
    ...
    collections: Map<
      string,
      { predicate: string; instanceFilter?: string; condition?: string }
    >;
  } | null> {
    try {
      // Resolve the exact SHACL shape URI from the name mapping to avoid overlapping class name issues
      const nameMapping = Literal.fromUrl(
        `literal://string:shacl://${className}`,
      );
      const shapeUriLinks = await this.get(
        new LinkQuery({
          source: nameMapping.toUrl(),
      ...
      const properties = new Map<
        string,
        { predicate: string; resolveLanguage?: string }
      >();
      const collections = new Map<
        string,
        { predicate: string; instanceFilter?: string }
      >();
      ...
      for (const propLink of propertyLinks) {
        ...
        const propDetailLinks = await this.get(
          new LinkQuery({
            source: propUri,
          }),
        );
        ...
        for (const detail of propDetailLinks) {
          if (detail.data.predicate === "sh://path") {
            ...
            resolveLanguage = detail.data.target?.replace(
              "literal://string:",
              "",
            );
          } else if (
            detail.data.predicate === "rdf://type" &&
            detail.data.target === "ad4m://Collection"
          ) {
            isCollection = true;
          } else if (
      ...
      if (constructorLinks.length > 0) {
        ...
        if (
          constructorTarget &&
          constructorTarget.startsWith("literal://string:")
        ) {
          try {
            const actionsJson = constructorTarget.substring(
            ...
            console.warn(
              `Failed to parse constructor actions for ${className}:`,
              e,
            );
          }
        }
  ...
  private generateSurrealInstanceQuery(metadata: {
    ...
    const whereConditions = metadata.requiredTriples
      .map((triple) => {
        const escapedPredicate = escapeSurrealString(triple.predicate);
        if (triple.target) {
          // Flag: must match both predicate AND exact target value
   ...
   */
  async getPropertyValueViaSurreal(
    baseExpression: string,
    className: string,
    propertyName: string,
  ): Promise<any> {
   ...
   */
  async getCollectionValuesViaSurreal(
    baseExpression: string,
    className: string,
    ...
    if (collMeta.condition && values.length > 0) {
      try {
        ...
        console.warn(
          `Failed to apply condition filter for ${collectionName}:`,
          error,
        );
      }
    }
    ...
    if (collMeta.instanceFilter) {
      try {
        const filterMetadata = await this.getSubjectClassMetadataFromSDNA(
          collMeta.instanceFilter,
        );
        if (!filterMetadata) {
          // Fallback to sequential checks if metadata isn't available
          return this.filterInstancesSequential(
            values,
            collMeta.instanceFilter,
          );
        }
  ...
  async batchCheckSubjectInstances(
    ...
    if (metadata.requiredTriples.length === 0) {
      const escapedExpressions = expressions
        .map((e) => `'${escapeSurrealString(e)}'`)
        .join(", ");
   ...
   */
  private async filterInstancesSequential(
    values: string[],
  ...
  private buildQueryFromTemplate(obj: object): string {
    ...
    (function (obj) {
      ...
      // Collect all string properties of the object in a list
      const _protoProps = getPropertiesMetadata(
        Object.getPrototypeOf(obj)?.constructor,
      );
      if (Object.keys(_protoProps).length > 0) {
        Object.keys(_protoProps).forEach((p) => properties.push(p));
      } else {
        properties.push(
      ...
      // Collect all collections of the object in a list
      const _protoCols = getRelationsMetadata(
        Object.getPrototypeOf(obj)?.constructor,
      );
      if (Object.keys(_protoCols).length > 0) {
        Object.keys(_protoCols)
          .filter((key) => key !== "isSubjectInstance")
          .forEach((c) => {
      ...
      let setFunctions = Object.getOwnPropertyNames(obj).filter(
        (key) =>
      ...
      setFunctions = setFunctions.concat(
        Object.getOwnPropertyNames(Object.getPrototypeOf(obj)).filter((key) => {
      ...
      let addFunctions = Object.getOwnPropertyNames(obj).filter(
        (key) =>
      ...
      addFunctions = addFunctions.concat(
        Object.getOwnPropertyNames(Object.getPrototypeOf(obj)).filter((key) => {
          const descriptor = Object.getOwnPropertyDescriptor(
            Object.getPrototypeOf(obj),
            key,
      ...
      let removeFunctions = Object.getOwnPropertyNames(obj).filter(
        (key) =>
          Object.prototype.hasOwnProperty.call(obj, key) &&
          typeof obj[key] === "function" &&
      ...
      removeFunctions = removeFunctions.concat(
        Object.getOwnPropertyNames(Object.getPrototypeOf(obj)).filter((key) => {
          const descriptor = Object.getOwnPropertyDescriptor(
            Object.getPrototypeOf(obj),
            key,
      ...
      let setCollectionFunctions = Object.getOwnPropertyNames(obj).filter(
        (key) =>
          Object.prototype.hasOwnProperty.call(obj, key) &&
          typeof obj[key] === "function" &&
          key.startsWith("set") &&
  ...
  async ensureSDNASubjectClass(jsClass: any): Promise<void> {
    ...
    const className =
      jsClass.className || jsClass.prototype?.className || jsClass.name;
    ...
    if (!jsClass.generateSHACL) {
      throw new Error(
        `Class ${jsClass.name} must have generateSHACL(). Use @ModelOptions decorator.`,
      );
    }

...
import { LinkExpression, LinkExpressionInput, LinkExpressionMutations, LinkInput, LinkInputMutations } from "../links/Links";
...
export default class PerspectiveResolver {
    ...
    @Mutation(returns => [LinkExpression])
    perspectiveRemoveLinks(@Arg('uuid') uuid: string, @Arg('links', type => [LinkInput]) links: LinkInput[], @PubSub() pubSub: any): LinkExpression[] {
        const l = new LinkExpression()
        l.author = 'did:ad4m:test'
        l.timestamp = Date.now()
        l.proof = testLink.proof
    ...
    perspectiveLinkMutations(@Arg('uuid') uuid: string, @Arg('mutations') mutations: LinkInputMutations, @PubSub() pubSub: any): LinkExpressionMutations {

...
export class LanguageClient {
  ...
  async writeSettings(
    ...
    const { languageWriteSettings } = unwrapApolloResult(
  ...
  async publish(
    ...
    const { languagePublish } = unwrapApolloResult(
      await this._apolloClient.mutate({
  ...
  async meta(address: string): Promise<LanguageMeta> {
    const { languageMeta } = unwrapApolloResult(
      await this._apolloClient.query({
  ...
  async source(address: string): Promise<string> {
    const { languageSource } = unwrapApolloResult(
      await this._apolloClient.query({
        query: gql`
          query languageSource($address: String!) {
  ...
  async remove(address: string): Promise<Boolean> {
    const { languageRemove } = unwrapApolloResult(
      await this._apolloClient.mutate({
        mutation: gql`
          mutation languageRemove($address: String!) {

...
const richMetadata: any = {
  properties: {
    ...
    category: {
      ...
      required: true,
      resolveLanguage: "file_storage",  // non-literal → expression URI stored
    },
    description: {
      name: 'description',
...
describe('buildSPARQLQuery — non-literal and flag properties', () => {
  ...
  it('does NOT use parse_literal for non-literal (resolveLanguage: language address) properties', () => {
    const meta: any = {
      properties: {
        avatar: { name: 'avatar', predicate: 'flux://avatar', required: true, resolveLanguage: "file_storage" },
      },
      relations: {},
    };

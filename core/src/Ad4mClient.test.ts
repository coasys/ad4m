...
describe("Ad4mClient", () => {
  ...
  beforeAll(async () => {
    // Each subscription test adds a listener to the shared PubSub EventEmitter;
    // raise the limit so Node doesn't emit a false-positive leak warning.
    EventEmitter.defaultMaxListeners = 30;

    const { port, stop } = await createGqlServer(4000);
    stopServer = stop;
  ...
  describe(".agent", () => {
    ...
    it("updatePublicPerspective() smoke test", async () => {
      ...
      expect(agent.perspective.links[0].data.target).toBe(
        "perspective://Qm34589a3ccc0",
      );
    });

    it("mutatePublicPerspective() smoke test", async () => {
      let additionLink = new Link({
        source: "root",
        target: "perspective://Qm34589a3ccc0",
      });
      ...
      const agent = await ad4mClient.agent.mutatePublicPerspective({
        additions: [additionLink],
        removals: [removalLink],
      } as LinkMutations);
      expect(agent.did).toBe("did:ad4m:test");
      expect(agent.perspective.links.length).toBe(1);
      expect(agent.perspective.links[0].data.source).toBe("root");
      expect(agent.perspective.links[0].data.target).toBe(
        "neighbourhood://Qm12345",
      );
    });
    ...
    it("entanglementProof() smoke tests", async () => {
      const addProof = await ad4mClient.agent.addEntanglementProofs([
        new EntanglementProofInput(
          "did:key:hash",
          "did-key-id",
          "ethereum",
          "ethAddr",
      ...
      const deleteProofs = await ad4mClient.agent.deleteEntanglementProofs([
        new EntanglementProofInput(
          "did:key:hash",
          "did-key-id",
          "ethereum",
      ...
      const preflight = await ad4mClient.agent.entanglementProofPreFlight(
        "ethAddr",
        "ethereum",
      );
    ...
    it("agentPermitCapability() smoke tests", async () => {
      const rand = await ad4mClient.agent.permitCapability(
        '{"requestId":"4f30e2e2-d307-4f2b-b0a0-6dac4ca4af26","auth":{"appName":"demo-app","appDesc":"demo-desc","appUrl":"demo-url","capabilities":[{"with":{"domain":"agent","pointers":["*"]},"can":["QUERY"]}]}}',
      );
  ...
  describe(".expression", () => {
    it("get() smoke test", async () => {
      ...
      const expression = await ad4mClient.expression.get(
        "neighbourhood://Qm123",
      );
      expect(expression).toBeDefined();
      expect(expression.author).toBe("did:ad4m:test");
      expect(expression.data).toBe(
        '{"type":"test expression","content":"test"}',
      );
    });
    ...
    it("getRaw() smoke test", async () => {
      ...
      const expressionRaw = await ad4mClient.expression.getRaw(
        "neighbourhood://Qm123",
      );
    ...
    it("create() smoke test", async () => {
      ...
      const address2 = await ad4mClient.expression.create(
        { content: "json" },
        "Qmabcdf",
      );
  ...
  describe(".langauges", () => {
    it("byAddress() smoke test", async () => {
      const language = await ad4mClient.languages.byAddress(
    ...
    it("publish() smoke test", async () => {
      ...
      expect(languageMeta.possibleTemplateParams).toStrictEqual(
        input.possibleTemplateParams,
      );
      ...
      expect(languageMeta.templateAppliedParams).toBe(
        JSON.stringify({ uuid: "asdfsdaf", name: "test template" }),
      );
    });
    ...
    it("meta() smoke test", async () => {
      ...
      expect(languageMeta.sourceCodeLink).toBe(
        "https://github.com/perspect3vism/ad4m",
      );
    });
  ...
  describe(".neighbourhood", () => {
    ...
    it("joinFromUrl() smoke test", async () => {
      const perspective = await ad4mClient.neighbourhood.joinFromUrl(
        "neighbourhood://Qm3sdf3dfwhsafd",
      );
    ...
    it("hasTelepresenceAdapter() smoke test", async () => {
      const result =
        await ad4mClient.neighbourhood.hasTelepresenceAdapter("01234");
      expect(result).toBe(true);
    });
    ...
    it("setOnlineStatus() smoke test", async () => {
      const result = await ad4mClient.neighbourhood.setOnlineStatus(
        "01234",
        testPerspective,
      );
      expect(result).toBe(true);
    ...
    it("setOnlineStatusU() smoke test", async () => {
      const result = await ad4mClient.neighbourhood.setOnlineStatusU(
        "01234",
        testUnsignedPerspective,
      );
      expect(result).toBe(true);
    });

    it("sendSignal() smoke test", async () => {
      const result = await ad4mClient.neighbourhood.sendSignal(
        "01234",
        "did:test:recipient",
        testPerspective,
      );
    ...
    it("sendSignaU() smoke test", async () => {
      const result = await ad4mClient.neighbourhood.sendSignalU(
        "01234",
        "did:test:recipient",
        testUnsignedPerspective,
      );
    ...
    it("sendBroadcast() smoke test", async () => {
      const result = await ad4mClient.neighbourhood.sendBroadcast(
        "01234",
        testPerspective,
      );
      expect(result).toBe(true);
    ...
    it("sendBroadcastU() smoke test", async () => {
      const result = await ad4mClient.neighbourhood.sendBroadcastU(
        "01234",
        testUnsignedPerspective,
      );
      expect(result).toBe(true);
    ...
    it("can be accessed via NeighbourhoodProxy", async () => {
      ...
      expect(await nh.sendSignal("did:test:recipient", testPerspective)).toBe(
        true,
      );
  ...
  describe(".perspective", () => {
    ...
    it("publishSnapshotByUUID() smoke test", async () => {
      const snapshotUrl =
        await ad4mClient.perspective.publishSnapshotByUUID("00004");
      expect(snapshotUrl).toBe("perspective://Qm12345");
    });

    it("queryLinks() smoke test", async () => {
      const links = await ad4mClient.perspective.queryLinks("000001", {
        source: "root",
      });
    ...
    it("queryProlog() smoke test", async () => {
      let result = await ad4mClient.perspective.queryProlog(
        "000001",
        "link(X, 2).",
      );
    ...
    it("addLink() smoke test", async () => {
      const link = await ad4mClient.perspective.addLink("00001", {
        source: "root",
        target: "lang://Qm123",
        predicate: "p",
      });
      ...
      const links = await ad4mClient.perspective.removeLinks("00001", [
        {
          ...
          timestamp: "",
          proof: { signature: "", key: "" },
          data: { source: "root", target: "lang://Qm123", predicate: "p" },
        },
        {
          author: "",
          timestamp: "",
    ...
    it("addLinkExpression() smoke test", async () => {
      ...
      const link = await ad4mClient.perspective.addLinkExpression(
        "00001",
        testLink,
      );
      expect(link.author).toBe("did:ad4m:test");
    ...
    it("removeLink() smoke test", async () => {
      const r = await ad4mClient.perspective.removeLink("00001", {
        author: "",
        timestamp: "",
        proof: { signature: "", key: "" },
        data: { source: "root", target: "none" },
    ...
    it("getSubjectData() smoke test", async () => {
      const result = await ad4mClient.perspective.getSubjectData(
        "00001",
        "Test",
        "test",
      );
  ...
  describe(".runtime", () => {
    ...
    it("addKnownLinkLanguageTemplates() smoke test", async () => {
      const r = await ad4mClient.runtime.addKnownLinkLanguageTemplates([
        "Qm1337",
      ]);
    ...
    it("removeKnownLinkLanguageTemplates() smoke test", async () => {
      const r = await ad4mClient.runtime.removeKnownLinkLanguageTemplates([
        "Qm12345abcdef",
      ]);
      expect(r).toStrictEqual([]);
    });
    ...
    it("addFriends() smoke test", async () => {
      const r = await ad4mClient.runtime.addFriends([
        "did:test:another_friend",
      ]);
    ...
    it("ververifyStringSignedByDid() smoke test", async () => {
      const verify = await ad4mClient.runtime.verifyStringSignedByDid(
        "did",
        "didSigningKeyId",
        "data",
        "signedData",
    ...
    it("friendSendMessage smoke test", async () => {
      ...
      link.proof = { signature: "asdfasdf", key: "asdfasdf" };
      await ad4mClient.runtime.friendSendMessage(
        "did:ad4m:test",
    ...
    it("messageOutbox smoke test", async () => {
      const sentMessages =
        await ad4mClient.runtime.messageOutbox("did:ad4m:test");
  ...
  describe("Ad4mClient subscriptions", () => {
    describe("ad4mClient without subscription", () => {
      ...
      it("agent subscribeAgentUpdated smoke test", async () => {
        ...
        ad4mClientWithoutSubscription.agent.addUpdatedListener(
        ...
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
        await ad4mClientWithoutSubscription.agent.updateDirectMessageLanguage(
          "lang://test",
        );
      ...
      it("agent subscribeAgentStatusChanged smoke test", async () => {
        ...
        ad4mClientWithoutSubscription.agent.addAgentStatusChangedListener(
      ...
      it("perspective subscribePerspectiveAdded smoke test", async () => {
        ...
        ad4mClientWithoutSubscription.perspective.addPerspectiveAddedListener(
          perspectiveAddedCallback,
        );
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
      ...
      it("perspective subscribePerspectiveUpdated smoke test", async () => {
        ...
        ad4mClientWithoutSubscription.perspective.addPerspectiveUpdatedListener(
          perspectiveUpdatedCallback,
        );
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
        ...
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
        await ad4mClientWithoutSubscription.perspective.update(
          "00006",
          "p-test2",
        );
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
      ...
      it("perspective subscribePerspectiveRemoved smoke test", async () => {
        ...
        ad4mClientWithoutSubscription.perspective.addPerspectiveRemovedListener(
          perspectiveRemovedCallback,
        );
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
    ...
    describe("ad4mClient with subscription", () => {
      ...
      it("agent subscribeAgentUpdated smoke test", async () => {
        const agentUpdatedCallback = jest.fn();
        ad4mClientWithSubscription.agent.addUpdatedListener(
          agentUpdatedCallback,
        );
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
        expect(agentUpdatedCallback).toBeCalledTimes(0);
        ...
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
        await ad4mClientWithSubscription.agent.updateDirectMessageLanguage(
          "lang://test",
        );
        expect(agentUpdatedCallback).toBeCalledTimes(1);
      });
      ...
      it("agent subscribeAgentStatusChanged smoke test", async () => {
        const agentStatusChangedCallback = jest.fn();
        ad4mClientWithSubscription.agent.addAgentStatusChangedListener(
          agentStatusChangedCallback,
        );
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
        expect(agentStatusChangedCallback).toBeCalledTimes(0);
      ...
      it("perspective subscribePerspectiveAdded smoke test", async () => {
        const perspectiveAddedCallback = jest.fn();
        ad4mClientWithSubscription.perspective.addPerspectiveAddedListener(
          perspectiveAddedCallback,
        );
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
        expect(perspectiveAddedCallback).toBeCalledTimes(0);
      ...
      it("perspective subscribePerspectiveUpdated smoke test", async () => {
        const perspectiveUpdatedCallback = jest.fn();
        ad4mClientWithSubscription.perspective.addPerspectiveUpdatedListener(
          perspectiveUpdatedCallback,
        );
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
        expect(perspectiveUpdatedCallback).toBeCalledTimes(0);
      ...
      it("perspective subscribePerspectiveRemoved smoke test", async () => {
        const perspectiveRemovedCallback = jest.fn();
        ad4mClientWithSubscription.perspective.addPerspectiveRemovedListener(
          perspectiveRemovedCallback,
        );
        await new Promise<void>((resolve) => setTimeout(resolve, 100));
        expect(perspectiveRemovedCallback).toBeCalledTimes(0);
  ...
  describe(".ai", () => {
    ...
    it.skip("setDefaultModel and getDefaultModel smoke test", async () => {
      ...
      const setResult = await ad4mClient.ai.setDefaultModel(
        modelType,
        modelName,
      );
      ...
      if (defaultModel.local.tokenizerSource) {
        ...
        expect(defaultModel.local.tokenizerSource.fileName).toBe(
          "tokenizer.json",
        );
      }
    ...
    it("removeTask()", async () => {
      const task = await ad4mClient.ai.removeTask(
        "task_id",
        "system prompt",
        [],
      );
    ...
    it("openTranscriptionStream(), closeTranscriptionStream(), feedTranscriptionStream() & aiTranscriptionText subscription", async () => {
      ...
      const streamId = await ad4mClient.ai.openTranscriptionStream(
        "model_id",
        streamCallback,
      );

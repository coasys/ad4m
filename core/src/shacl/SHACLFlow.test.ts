...
describe('SHACLFlow', () => {
  ...
  describe('fromLinks()', () => {
    ...
    it('reconstructs flow after typed-literal migration rewrites literal:string: targets to plain values', () => {
      // Mirror what `migrate_iri_literals_to_typed_literals` does on the Rust
      // side: strip `literal:string:` prefixes on stateName / stateValue /
      // actionName link targets, and percent-decode the payload for
      // stateCheck / flowable / startAction / transitionActions. The rest of
      // fromLinks() must still recover the original values.
      const original = new SHACLFlow('TODO', 'todo://');
      original.flowable = 'any';
      original.startAction = [
        { action: 'addLink', source: 'this', predicate: 'todo://state', target: 'todo://ready' }
      ...
      const raw = original.toLinks();
      const plainLiteralPreds = new Set([
        "ad4m://stateName",
        "ad4m://stateValue",
        "ad4m://actionName",
      ]);
      const jsonPayloadPreds = new Set([
        "ad4m://flowable",
        "ad4m://startAction",
        "ad4m://stateCheck",
      ...
      const migrated = raw.map(l => {
        if (plainLiteralPreds.has(l.predicate) && l.target.startsWith("literal:string:")) {
          return { ...l, target: decodeURIComponent(l.target.slice("literal:string:".length)) };
        }
        if (plainLiteralPreds.has(l.predicate) && l.target.startsWith("literal:number:")) {
          return { ...l, target: l.target.slice("literal:number:".length) };
        }
        if (jsonPayloadPreds.has(l.predicate) && l.target.startsWith("literal:string:")) {
          return { ...l, target: decodeURIComponent(l.target.slice("literal:string:".length)) };
        }
        return l;
      });

      const reconstructed = SHACLFlow.fromLinks(migrated, 'todo://TODOFlow');

      expect(reconstructed.name).toBe('TODO');
      expect(reconstructed.flowable).toBe('any');
      expect(reconstructed.states.length).toBe(2);
      expect(reconstructed.states.find(s => s.name === 'ready')?.value).toBe(0);
      expect(reconstructed.states.find(s => s.name === 'done')?.value).toBe(1);
      expect(reconstructed.transitions.length).toBe(1);
      expect(reconstructed.transitions[0].actionName).toBe('Complete');
      expect(reconstructed.transitions[0].fromState).toBe('ready');
      expect(reconstructed.transitions[0].toState).toBe('done');
    });
  });

import { SHACLFlow } from "./SHACLFlow";
...
describe("SHACLFlow", () => {
  describe("basic construction", () => {
    ...
    it("generates correct transition URIs", () => {
      const flow = new SHACLFlow("TODO", "todo://");
      expect(flow.transitionUri("ready", "doing")).toBe(
        "todo://TODO.readyTodoing",
      );
    });
  });
  ...
  describe("toLinks()", () => {
    it("serializes flow to links", () => {
      ...
      const typeLink = links.find(
        (l) => l.predicate === "rdf://type" && l.target === "ad4m://Flow",
      );
      ...
      const startActionLink = links.find(
        (l) => l.predicate === "ad4m://startAction",
      );
      ...
      const transitionLink = links.find(
        (l) => l.predicate === "ad4m://hasTransition",
      );

...
// ---------------------------------------------------------------------------
// Graph DAG — ALDK `GraphCommitCapability` + `GraphSyncCapability`.
// Per-graph head tracking delegated from the executor. The executor computes
// diffs and commit hashes (it has the SPARQL store); the link language stores
// and replicates head pointers so they gossip to peers.
// ---------------------------------------------------------------------------
impl Language {
    pub async fn graph_set_head(
        &mut self,
        graph_id: &str,
        commit_iri: &str,
        parent_iris: &[String],
    ) -> Result<(), AnyError> {
        if !self.has(Capability::GraphSetHead) {
            return Ok(());
        }
        let controller = LanguageController::global_instance();
        let graph_id_json = serde_json::to_string(graph_id)?;
        let commit_iri_json = serde_json::to_string(commit_iri)?;
        let parents_json = serde_json::to_string(parent_iris)?;
        let script = format!(
            r#"JSON.stringify((await language.graphSetHead({}, {}, {})) ?? null)"#,
            graph_id_json, commit_iri_json, parents_json
        );
    ...
    pub async fn graph_current_revision(
        &mut self,
        graph_id: &str,
    ) -> Result<Option<String>, AnyError> {
        if !self.has(Capability::GraphCurrentRevision) {
            return Ok(None);
        }
        let controller = LanguageController::global_instance();
        let graph_id_json = serde_json::to_string(graph_id)?;
        let script = format!(
            r#"JSON.stringify((await language.graphCurrentRevision({})) ?? null)"#,
            graph_id_json
        );
    ...
    pub async fn graph_heads(&mut self) -> Result<Vec<(String, String)>, AnyError> {
        ...
        let controller = LanguageController::global_instance();
        let script = r#"JSON.stringify((await language.graphHeads()) ?? [])"#;

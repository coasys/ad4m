...
mod tests {
    ...
    #[test]
    fn test_publish_params_deserialize() {
        let json = r#"{
            "perspective_uuid": "abc-123",
            "link_language_template": "QmzSYwdnDDbtgry2DmiqYpVtoPP75MabguWZUy1Ene7NzQAQWz2",
            "name": "My SoA Memory"
        }"#;
        let params: NeighbourhoodPublishParams = serde_json::from_str(json).unwrap();
        assert_eq!(params.perspective_uuid, "abc-123");
        assert_eq!(
            params.link_language_template,
    ...
    #[test]
    fn test_join_params_deserialize() {
        let json =
            r#"{"url": "neighbourhood://QmzSYwdnHx136vGXLuC4xWtaJSCmKGobrwoKTGh2aopxDkEEpbZ"}"#;
        let params: NeighbourhoodJoinParams = serde_json::from_str(json).unwrap();
        assert_eq!(
            params.url,
            "neighbourhood://QmzSYwdnHx136vGXLuC4xWtaJSCmKGobrwoKTGh2aopxDkEEpbZ"
        );
    }

    #[test]
    fn test_template_data_has_unique_uid() {
        // Simulate what clone_link_language builds internally
        let uid1 = uuid::Uuid::new_v4().to_string();
        let uid2 = uuid::Uuid::new_v4().to_string();
        assert_ne!(uid1, uid2, "Each clone should get a unique UID");

        let mut template_map = serde_json::Map::new();
        template_map.insert("uid".to_string(), serde_json::Value::String(uid1.clone()));
        template_map.insert(
            "name".to_string(),
            serde_json::Value::String("Test Neighbourhood".to_string()),
        );

        assert_eq!(template_map.get("uid").unwrap().as_str().unwrap(), &uid1);
        assert_eq!(
            template_map.get("name").unwrap().as_str().unwrap(),
            "Test Neighbourhood"
        );
    }

    #[test]
    fn test_publish_response_json_structure() {
        // Verify the success response has all expected fields
        let response = json!({
            "success": true,
            "neighbourhood_url": "neighbourhood://QmTest123",
            "perspective_uuid": "abc-123",
            "cloned_link_language": "QmCloned456",
            "name": "My Neighbourhood",
            "message": "Perspective published as neighbourhood. Share the neighbourhood_url for others to join."
        });

        assert!(response["success"].as_bool().unwrap());
        assert!(response["neighbourhood_url"]
            .as_str()
            .unwrap()
            .starts_with("neighbourhood://"));
        assert!(!response["cloned_link_language"]
            .as_str()
            .unwrap()
            .is_empty());
        assert_eq!(response["name"].as_str().unwrap(), "My Neighbourhood");
    }

    #[test]
    fn test_templates_response_json_structure() {
        // Verify the templates response has rich template objects (not just strings)
        let template = json!({
            "address": "QmzSYwdnDDbtgry2DmiqYpVtoPP75MabguWZUy1Ene7NzQAQWz2",
            "name": "perspective-diff-sync",
            "description": "Holochain-based P2P sync engine",
        ...
        assert!(t["address"].as_str().is_some());

...
// against hand-built link bags, `build_flow_contexts` against hand-built
// records + catalogues, `scope_subject` against `Scope` values, and the
// rendering helpers against constructed `FlowContext`s. What none of them
// touch is the composition — the real path a live extraction pass walks:
//
//   real perspective
//     ↳ SHACLFlow definition seeded as actual links (writer half of the
//         parser round-trip: `parse_flow_to_links` → `add_link`)
//     ↳ FlowInstance minted via the runtime primitive
//         (`mint_flow_instance` — same call the engine will make)
//     ↳ `gather_active_flow_contexts` walking the perspective's
//         SPARQL/Prolog stores through the model_query layer
//     ↳ resulting `FlowContext[]` fed to `build_interpretation_input`,
//         which is what `run.rs` does after slice 10.3c wired both call
//         sites.
//
// This test is the onion shell for the current cut — the natural stopping
// point Nico asked for on 2026-08-26 evening ("cut based on what we can
// test really, stack it like onions"). If it stays green, slice 10.3c's
// `run.rs` wiring is by construction correct: the two lines it replaced
// (`build_interpretation_input(shapes, transcript, &existing, &[])`) now
// receive the exact `Vec<FlowContext>` this test builds, and slice 10.2's
// unit tests already cover what happens on that value inside the prompt
// builder.
//
// No LLM is spun up — `setup_perspective_no_llm` gives a real
// `PerspectiveInstance` with SPARQL, Prolog, and the SDNA/model_query
// stack attached. The only piece that would need Ollama is the AIService,
// which none of the code paths under test touch.
#[cfg(test)]
mod e2e_tests {
    use super::*;
    ...
    async fn gather_active_flow_contexts_wires_definition_and_instance_e2e() {
        ...
        let parsed: serde_json::Value =
            serde_json::from_str(&prompt).expect("prompt is valid JSON");

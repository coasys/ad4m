...
async fn run_engine_proposal_pass_e2e() {
    ...
    let before = run_engine_proposal_pass(&mut f.perspective, None, &f.ctx, None, &[]).await;
    assert!(
        before.is_empty(),
    ...
    f.seed_task("ad4m://task/1", "Onboard Ana").await;
    let minted = run_engine_proposal_pass(&mut f.perspective, None, &f.ctx, None, &[]).await;
...
async fn run_engine_proposal_pass_is_idempotent_e2e() {
    ...
    let first = run_engine_proposal_pass(&mut f.perspective, None, &f.ctx, None, &[]).await;
    assert_eq!(first.len(), 1, "first pass must mint one proposal");

    let second = run_engine_proposal_pass(&mut f.perspective, None, &f.ctx, None, &[]).await;
    assert!(
        second.is_empty(),
...
/// Scoped pass on a URI with no flow instances → empty return. Proves
/// the scope narrows the sweep: a flow instance exists at BASE_URI but
/// not at the queried scope, so the engine finds nothing.
#[tokio::test(flavor = "multi_thread")]
async fn run_engine_proposal_pass_scoped_miss_returns_empty() {
    let mut f = seed_delivery_fixture().await;
    f.seed_task("ad4m://task/1", "Onboard Ana").await;

    let miss_scope = Scope::Model {
        model: "ns://Task".to_string(),
        id: "ad4m://nonexistent/scope".to_string(),
        field: None,
    };
    let result =
        run_engine_proposal_pass(&mut f.perspective, Some(&miss_scope), &f.ctx, None, &[]).await;
    assert!(
        result.is_empty(),
        "scoped miss must return empty, got {result:?}"
    );
}

...
/// A transcript that offers **nothing to classify** as a Task — no assignment,
/// no first-person commitment, just a factual quip. With Task the only class
/// on the surface, the harness should terminate without ever calling
/// `Task_propose_create`. Result: zero persisted Task instances.
///
...
async fn harness_read_only_transcript_produces_no_writes() {
    let (p, shapes, placements) = run_harness_e2e(
        &[("Task", TASK_SDNA)],
        &[("Nico", "Coffee's cold again.")],
        8,
    )
    .await;

    // No _propose_create should have fired → no bases → no persisted instances.
    assert!(
        placements.is_empty(),
        "harness should not have written any Task from a Task-less transcript; got {placements:?}"
    );
    let counts = graph_count_by_type(&p, &shapes).await;
    assert!(
        counts.is_empty(),
        "graph should have no Task instances after a read-only pass; got {counts:?}"
    );
}

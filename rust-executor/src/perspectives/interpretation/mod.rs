...
//!   2. Render those, plus each class's `interpretation_hint` + fields and the
//!      transcript, into a single prompt (`build_interpretation_input`).
...
//! in-file; the real-LLM end-to-end suite lives in `interpretation_e2e.rs`, with
//! shared fixtures/harness in `interpretation_test_support.rs`.

...
use super::pool_trait::{FilteredPool, EnginePoolState, PoolUtils, EmbeddingCache};
use crate::perspectives::sdna::{get_static_infrastructure_facts, get_sdna_facts};
...
impl FilteredPool for SdnaPrologPool {
    ...
    async fn populate_from_complete_data(
        ...
        let facts = self.create_sdna_only_facts(all_links, neighbourhood_author).await?;
        
        log::info!("📊 SDNA UPDATE: Pool updating engines with {} facts", facts.len());
...
impl SdnaPrologPool {
    ...
    fn is_subject_class_query(&self, query: &str) -> bool {
        ...
        let is_subject_class = has_subject_class_predicates && !has_link_data_predicates && !has_instance_predicates;
    ...
    async fn create_sdna_only_facts(
        ...
        log::info!("📊 SDNA FACT CREATION: Infrastructure facts: {}, SDNA facts: {}", 
            sdna_lines.len(), sdna_facts.len());
        ...
        log::info!("📊 SDNA FACT CREATION: Total SDNA-only facts: {} (no link data included)", 
            sdna_lines.len());
...
mod tests {
    ...
    async fn test_sdna_only_population() {
        ...
        let result = sdna_pool.populate_from_complete_data("facts".to_string(), &test_links, None).await;
        assert!(result.is_ok());

pub use super::embedding_cache::EmbeddingCache;
...
pub struct EnginePoolState {
    ...
    pub current_owner_did: Option<String>,
...
impl PoolUtils {
    ...
    pub async fn replace_embedding_url(
        query: String,
    ...
    pub async fn replace_embedding_url_in_value_recursively(
        ...
        match value {
            ...
            Term::List(list) => {
                ...
                for item in list {
                    Box::pin(Self::replace_embedding_url_in_value_recursively(
                        item,
                        embedding_cache,
                    ))
                    .await;
    ...
    pub async fn preprocess_program_lines(
        ...
        let futures = lines.into_iter().map(|line| {
            ...
            async move { Self::replace_embedding_url(line_clone, &cache_clone).await }
        });
    ...
    pub async fn handle_engine_error(
        ...
        Err(anyhow!(
            "{} engine failed and was invalidated: {}",
            pool_name,
            error
        ))
    }

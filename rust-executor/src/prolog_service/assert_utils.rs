...
pub fn extract_assert_statements(query: &str) -> Vec<String> {
    ...
    if is_single_assert_statement(query_without_period) && !has_comma_outside_parens(query_without_period) {
...
mod tests {
    ...
    fn test_is_assert_query() {
        assert!(is_assert_query("assert_link_and_triple(\"a\", \"b\", \"c\", 123, \"author\")."));
    ...
    fn test_extract_assert_statements() {
        ...
        let single_query = "assert_link_and_triple(\"user1\", \"likes\", \"item1\", \"123\", \"author1\").";

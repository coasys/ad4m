...
> **All AD4M URIs are valid IRIs.** The `literal:` format ensures that no AD4M URI requires escaping or transformation to be used as an RDF IRI.
...
Values are encoded using RFC 3986 percent-encoding via `encodeRFC3986URIComponent()`. The `literal:` scheme (without `//`) is RFC 3986 compliant — there is no authority component. The `Literal.fromUrl()` constructor MUST reject URLs that do not start with `literal:`.

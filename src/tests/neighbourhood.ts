    ...
    return () => {
        describe('Neighbourhood', () => {
            it('can publish and join locally @alice', async () => {
                ...
                const socialContext = await ad4mClient.languages.applyTemplateAndPublish(DIFF_SYNC_OFFICIAL, JSON.stringify({network_seed: uuidv4(), name: "Alice's perspective-diff-sync"}));
            ...
            it('can be created by Alice and joined by Bob', async () => {
                ...
                const socialContext = await alice.languages.applyTemplateAndPublish(DIFF_SYNC_OFFICIAL, JSON.stringify({network_seed: uuidv4(), name: "Alice's neighbourhood with Bob"}));

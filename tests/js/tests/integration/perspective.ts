  ...
  return () => {
    describe("Perspectives", () => {
      it("can create, get & delete perspective", async () => {
        ...
        const update = await ad4mClient!.perspective.update(
          create.uuid,
          "updated-test",
        );
        ...
        const perspectiveSnaphot = await ad4mClient.perspective.snapshotByUUID(
          update.uuid,
        );
        expect(perspectiveSnaphot!.links.length).to.equal(0);

        const deletePerspective = await ad4mClient!.perspective.remove(
          update.uuid,
        );
      ...
      it("can CRUD local perspective links", async () => {
        ...
        const linkAdd = await create.add(
          new Link({
      ...
      it(`doesn't error when duplicate entries passed to removeLinks`, async () => {
        const ad4mClient = testContext.ad4mClient!;
        const perspective = await ad4mClient.perspective.add(
      ...
      it("test local perspective links - time query", async () => {
        ...
        let addLink = await ad4mClient!.perspective.addLink(
          create.uuid,
          new Link({
            source: "lang://test",
            target: "lang://test-target",
            predicate: "lang://predicate",
          }),
        );
        await sleep(10);
        let addLink2 = await ad4mClient!.perspective.addLink(
          create.uuid,
          new Link({
            source: "lang://test",
            target: "lang://test-target2",
            predicate: "lang://predicate",
          }),
        );
        await sleep(10);
        let addLink3 = await ad4mClient!.perspective.addLink(
          create.uuid,
          new Link({
            source: "lang://test",
            target: "lang://test-target3",
            predicate: "lang://predicate",
          }),
        );
        await sleep(10);
        let addLink4 = await ad4mClient!.perspective.addLink(
          create.uuid,
          new Link({
            source: "lang://test",
            target: "lang://test-target4",
            predicate: "lang://predicate",
          }),
        );
        await sleep(10);
        let addLink5 = await ad4mClient!.perspective.addLink(
          create.uuid,
          new Link({
            source: "lang://test",
            target: "lang://test-target5",
            predicate: "lang://predicate",
          }),
        );

        // Get all the links
        let queryLinksAll = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({
            source: "lang://test",
            fromDate: new Date(new Date(addLink.timestamp).getTime()),
            untilDate: new Date(),
          }),
        );
        ...
        let queryLinksAsc = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({
            source: "lang://test",
            fromDate: new Date(),
        ...
        // Get 3 of the links in descending order
        let queryLinksDesc = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({
            source: "lang://test",
            fromDate: new Date("August 19, 1975 23:15:30"),
            untilDate: new Date(),
            limit: 3,
          }),
        );
        ...
        let queryLinks = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({
            source: "lang://test",
            fromDate: new Date(new Date(addLink2.timestamp).getTime() - 1),
            untilDate: new Date(),
          }),
        );
        ...
        let queryLinksLimited = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({
            source: "lang://test",
            fromDate: new Date(new Date(addLink2.timestamp).getTime() - 1),
        ...
        let queryLinksFirst = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({
            source: "lang://test",
            fromDate: new Date(addLink.timestamp),
      ...
      it("test local perspective links", async () => {
        ...
        let queryLinks = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({ source: "lang://test" }),
        );
        ...
        let queryLinksTarget = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({ target: "lang://test-target" }),
        );
        ...
        let queryLinksPredicate = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({ predicate: "lang://predicate" }),
        );
        expect(queryLinksPredicate.length).to.equal(1);
        expect(queryLinksPredicate[0].data.target).to.equal(
          "lang://test-target",
        );
        ...
        const updateLink = await ad4mClient.perspective.updateLink(
          create.uuid,
          addLink,
          new Link({
            source: "lang://test2",
            target: "lang://test-target2",
            predicate: "lang://predicate2",
          }),
        );
        expect(updateLink.data.target).to.equal("lang://test-target2");
        expect(updateLink.data.source).to.equal("lang://test2");

        const perspectiveSnaphotLinkUpdate =
          await ad4mClient.perspective.snapshotByUUID(create.uuid);
        expect(perspectiveSnaphotLinkUpdate!.links.length).to.equal(1);

        //Test cannot get old link
        let queryLinksOld = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({ source: "lang://test" }),
        );
        ...
        let queryLinksUpdated = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({ source: "lang://test2" }),
        );
        expect(queryLinksUpdated.length).to.equal(1);

        const deleteLink = await ad4mClient!.perspective.removeLink(
          create.uuid,
          updateLink,
        );
        expect(deleteLink).to.equal(true);

        let queryLinksDeleted = await ad4mClient!.perspective.queryLinks(
          create.uuid,
          new LinkQuery({ source: "lang://test2" }),
        );
      ...
      it("subscriptions", async () => {
        ...
        ad4mClient.perspective.addPerspectiveUpdatedListener(
          perspectiveUpdated,
        );
        const perspectiveRemoved = sinon.fake();
        ad4mClient.perspective.addPerspectiveRemovedListener(
          perspectiveRemoved,
        );
        ...
        await ad4mClient.perspective.addPerspectiveLinkAddedListener(p1.uuid, [
          linkAdded,
        ]);
        const linkRemoved = sinon.fake();
        await ad4mClient.perspective.addPerspectiveLinkRemovedListener(
          p1.uuid,
          [linkRemoved],
        );
        const linkUpdated = sinon.fake();
        await ad4mClient.perspective.addPerspectiveLinkUpdatedListener(
          p1.uuid,
          [linkUpdated],
        );

        const linkExpression = await ad4mClient.perspective.addLink(p1.uuid, {
          source: "ad4m://root",
          target: "lang://123",
        });
        await sleep(1000);
        ...
        const updatedLinkExpression = await ad4mClient.perspective.updateLink(
          p1.uuid,
          linkExpression,
          { source: "ad4m://root", target: "lang://456" },
        );
        await sleep(1000);
        expect(linkUpdated.called).to.be.true;
        expect(linkUpdated.getCall(0).args[0].newLink).to.eql(
          updatedLinkExpression,
        );
      ...
      it.skip("can run Prolog queries", async () => {
        ...
        expect(
          await p.infer('reachable("ad4m://root", "todo-ontology://is-todo")'),
        ).to.be.true;

        const linkResult = await p.infer(
          'link(X, _, "todo-ontology://is-todo", Timestamp, Author).',
        );
    ...
    describe.skip("Batch Operations", () => {
      ...
      it("should support batch operations with multiple commands", async () => {
        ...
        const removedLink = links.find(
          (l) => l.data.source === "test://source0",
        );
    ...
    describe("PerspectiveProxy", () => {
      ...
      it("can do singleTarget operations", async () => {
        ...
        expect(await proxy.getSingleTarget(new LinkQuery(link1))).to.equal(
          "test://target1",
        );
      ...
      it.skip("can subscribe to Prolog query results", async () => {
        ...
        // Create subscription
        const subscription = await (proxy as any).subscribeInfer(
          'triple(X, _, "todo-ontology://is-todo").',
        );
        ...
        expect(latestResult.map((r: any) => r.X)).to.include(
          "note-ipfs://Qm123",
        );
        expect(latestResult.map((r: any) => r.X)).to.include(
          "note-ipfs://Qm456",

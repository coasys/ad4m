  ...
  return () => {
    describe("Expressions", () => {
      ...
      before(async () => {
        ...
        const publish = await ad4mClient.languages.publish(
          "./languages/note-store/build/bundle.js",
          {
            name: "note-store",
            description: "A test language for saving simple strings",
      ...
      it("can getManyExpressions()", async () => {
        ...
        const agentAndNull = await ad4mClient.expression.getMany([
          me.did,
          "lang://getNull",
          me.did,
        ]);
      ...
      it("can getRaw() my agent expression", async () => {
        ...
        expect(JSON.parse(agent).data.directMessageLanguage).to.be.equal(
          me.directMessageLanguage,
        );
      });
      ...
      it("can get expression from cache", async () => {
        ...
        const objExpr = await ad4mClient.expression.create(
          { key: "value" },
          noteLangAddress,
        );
        ...
        expect(exprObjCacheHit.data).to.be.equal(
      ...
      it("can use expression interactions", async () => {
        ...
        //Publish mocking interactions language so it can be used
        const publish = await ad4mClient.languages.publish(
          "./languages/test-language/build/bundle.js",
          {
            name: "test-language",
            description: "A test language for interactions",
          } as LanguageMetaInput,
        );
        ...
        const interactionCall = new InteractionCall("modify", {
          newValue: "modified note",
        });
        const result = await ad4mClient.expression.interact(
          exprAddr,
          interactionCall,
        );

...
async function convertAudioToPCM(audioFilePath: string): Promise<Float32Array> {
  ...
  return new Float32Array(
    pcmData.buffer,
    pcmData.byteOffset,
    pcmData.byteLength / Float32Array.BYTES_PER_ELEMENT,
  );
}
  ...
  return () => {
    describe("AI service", () => {
      ...
      it("can perform Model CRUD operations", async () => {
        ...
        const addedApiModel = models.find(
          (model) => model.name === "TestApiModel",
        );
        expect(addedApiModel).to.exist;
        expect(addedApiModel?.id).to.equal(addApiResult);
        expect(addedApiModel?.api?.baseUrl).to.equal(
        ...
        const addedLocalModel = models.find(
          (model) => model.name === "TestLocalModel",
        );
        ...
        expect(addedLocalModel?.local?.tokenizerSource?.repo).to.equal(
          "test-repo",
        );
        expect(addedLocalModel?.local?.tokenizerSource?.revision).to.equal(
          "main",
        );
        expect(addedLocalModel?.local?.tokenizerSource?.fileName).to.equal(
          "tokenizer.json",
        );
        ...
        const removeApiResult = await ad4mClient.ai.removeModel(
          addedApiModel!.id,
        );
        expect(removeApiResult).to.be.true;

        const removeLocalResult = await ad4mClient.ai.removeModel(
          addedLocalModel!.id,
        );
        ...
        const removedApiModel = updatedModels.find(
          (model) => model.name === "TestApiModel",
        );
        expect(removedApiModel).to.be.undefined;

        const removedLocalModel = updatedModels.find(
          (model) => model.name === "TestLocalModel",
        );
      ...
      it("can update model", async () => {
        ...
        const models = await ad4mClient.ai.getModels();
        const addedModel = models.find(
          (model) => model.name === "TestUpdateModel",
        );
        ...
        let error = {} as any;
        try {
          updateResult = await ad4mClient.ai.updateModel(
            addedModel!.id,
            bogusModelUrls,
          );
        } catch (e) {
      ...
      it.skip("can update model and verify it works", async () => {
        ...
        // Update the model
        const updateResult = await ad4mClient.ai.updateModel(
          modelId,
          updatedModel,
        );
        expect(updateResult).to.be.true;
      ...
      it.skip("can do Tasks CRUD", async () => {
        ...
        expect(newTask.promptExamples).to.deep.equal([
          { input: "Test input", output: "Test output" },
        ]);
        ...
        expect(
          tasks.find((task) => task.taskId === newTask.taskId),
        ).to.deep.equal(newTask);
        ...
        expect(updatedTask.promptExamples).to.deep.equal([
          { input: "Updated input", output: "Updated output" },
        ]);
        ...
        expect(tasksAfterRemoval.find((task) => task.taskId === newTask.taskId))
          .to.be.undefined;
      }).timeout(900000);
      ...
      it.skip("can prompt a task", async () => {
          ...
          [
            {
              ...
              output: "works. Also that is Paris",
            },
            {
              input: "What's the largets planet in our solar system?",
              output: "works. That is Jupiter.",
            },
          ],
        );
        ...
        // Prompt the task
        const promptResult = await ad4mClient.ai.prompt(
          newTask.taskId,
      ...
      it.skip("can do audio to text transcription", async () => {
        ...
        const feedStream = ad4mClient.ai.feedTranscriptionStream.bind(
          ad4mClient.ai,
        ) as unknown as (
          streamIds: string | string[],
          audio: number[],
        ...
        expect(transcribedText).to.include(
          "If you can read this, transcription is working.",
        );
      ...
      it.skip("can do fast (word-by-word) audio transcription", async () => {
        ...
        expect(transcribedWords.join(" ")).to.include(
          "If you can read this, transcription is working",
        );
      ...
      it.skip("can do fast and accurate transcription simultaneously", async () => {
        ...
        const fastStreamId = await ad4mClient.ai.openTranscriptionStream(
          "whisper_tiny",
          (text) => {
        ...
        const accurateStreamId = await ad4mClient.ai.openTranscriptionStream(
          "whisper_small",
          (text) => {
        ...
        await waitForTranscription(
          () =>
            fastTranscription.length > 0 && accurateTranscription.length > 0,
        );
        ...
        expect(accurateTranscription).to.include(
          "If you can read this, transcription is working",
        );

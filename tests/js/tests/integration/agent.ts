    ...
    describe("basic agent operations", () => {
      it("can get and create agent store", async () => {
        ...
        it("can get and create agent expression profile", async () => {
          ...
          await sleep(500);
          expect(agentUpdated.calledOnce).to.be.true;
          expect(agentUpdated.getCall(0).args[0]).to.deep.equal(
            updatePerspective,
          );

          const updatePublicLanguage =
            await ad4mClient.agent.updateDirectMessageLanguage("newlang");
          expect(currentAgent.perspective).not.to.be.undefined;
          expect(updatePublicLanguage.perspective!.links.length).to.equal(1);
          expect(updatePublicLanguage.directMessageLanguage).to.equal(
            "newlang",
          );

          await sleep(500);
          expect(agentUpdated.calledTwice).to.be.true;
          expect(agentUpdated.getCall(1).args[0]).to.deep.equal(
          ...
          expect(currentAgentPostUpdate.directMessageLanguage).to.equal(
            "newlang",
          );
      ...
      it("can create entanglementProofPreFlight", async () => {
        ...
        const verify = await ad4mClient.runtime.verifyStringSignedByDid(
          preFlight.did,
          preFlight.didSigningKeyId,
          "ethAddr",
          preFlight.deviceKeySignedByDid,
        );
        ...
        const addProof = await ad4mClient.agent.addEntanglementProofs([
          preFlight as EntanglementProofInput,
        ]);
        ...
        const getProofsPostDelete =
          await ad4mClient.agent.getEntanglementProofs();

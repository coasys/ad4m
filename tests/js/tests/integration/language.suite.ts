...
export default function languageTests(testContext: TestContext) {
  return () => {
    describe("with a perspective-diff-sync templated by Alice", () => {
      ...
      let sourceLanguageMeta: LanguageMetaInput = new LanguageMetaInput(
        "Newly published perspective-diff-sync",
        "..here for you template",
      );
      sourceLanguageMeta.possibleTemplateParams = [
        "uid",
        "description",
        "name",
      ];
      ...
      it("Alice can get the source of her own templated language", async () => {
        const sourceFromAd4m = await ad4mClient.languages.source(
          sourceLanguage.address,
        );
        const sourceFromFile = fs
          .readFileSync(
            path.join(
              __dirname,
              "../../tst-tmp/languages/perspective-diff-sync/build/bundle.js",
      ...
      it("Alice can install her own published language", async () => {
        const install = await ad4mClient.languages.byAddress(
          sourceLanguage.address,
        );
      ...
      it("Alice can install her own non HC published language", async () => {
        let sourceLanguageMeta: LanguageMetaInput = new LanguageMetaInput(
          "Newly published perspective-language",
          "..here for you template",
        );
        let socialContextData = fs
          .readFileSync(
      ...
      it("Alice can get her own templated perspective-diff-sync and it provides correct meta data", async () => {
        ...
        const foundSourceLanguageMeta = await ad4mClient.expression.get(
          `lang://${sourceLanguage.address}`,
        );
        ...
        expect(sourceLanguageMetaData.name).to.be.equal(
          sourceLanguageMeta.name,
        );
        expect(sourceLanguageMetaData.description).to.be.equal(
          sourceLanguageMeta.description,
        );
      });

      it("can publish and template a non-Holochain language and provide correct meta data", async () => {
        const noteMetaInfo = new LanguageMetaInput(
          "Newly published note language",
          "Just to test non-HC language work as well",
        );
        ...
        const sourceLanguageMetaNonHC = await ad4mClient.expression.get(
          `lang://${canPublishNonHolochainLang.address}`,
        );
        expect(sourceLanguageMetaNonHC.proof.valid).to.be.true;
        const sourceLanguageMetaNonHCData = JSON.parse(
          sourceLanguageMetaNonHC.data,
        );
        expect(sourceLanguageMetaNonHCData.name).to.be.equal(noteMetaInfo.name);
        expect(sourceLanguageMetaNonHCData.description).to.be.equal(
      ...
      it("Bob can not get/install Alice's language since he doesn't trust her yet", async () => {
        ...
        expect(error.toString()).to.contain(
          `ApolloError: Language not created by trusted agent: ${(await ad4mClient.agent.me()).did} and is not templated... aborting language install. Language metadata: ${stringify(sourceLanguageMeta)}`,
        );
      });
      ...
      describe("with Bob having added Alice to list of trusted agents", () => {
        ...
        before(async () => {
          ...
          const aliceTrusted = await bobAd4mClient.runtime.addTrustedAgents([
            aliceDid.did,
          ]);
        ...
        it("Bob can template Alice's perspective-diff-sync, and alice can install", async () => {
          ...
          applyTemplateFromSource =
            await bobAd4mClient.languages.applyTemplateAndPublish(
              sourceLanguage.address,
              JSON.stringify({
          ...
          expect(applyTemplateFromSource.name).to.be.equal(
            "Bob's templated perspective-diff-sync",
          );
          expect(applyTemplateFromSource.address).not.be.equal(
            sourceLanguage.address,
          );

          //Get language meta for above language and make sure it is correct
          const langExpr = await bobAd4mClient.expression.get(
            `lang://${applyTemplateFromSource.address}`,
          );
          expect(langExpr.proof.valid).to.be.true;
          const meta = await bobAd4mClient.languages.meta(
            applyTemplateFromSource.address,
          );
          expect(meta.name).to.be.equal(
            "Bob's templated perspective-diff-sync",
          );
          expect(meta.description).to.be.equal("..here for you template");
          expect(meta.author).to.be.equal(
            (await bobAd4mClient.agent.status()).did,
          );
          expect(meta.templateAppliedParams).to.be.equal(
            JSON.stringify({
              name: "Bob's templated perspective-diff-sync",
              uid: "2eebb82b-9db1-401b-ba04-1e8eb78ac84c",
            }),
          );
          expect(meta.templateSourceLanguageAddress).to.be.equal(
            sourceLanguage.address,
          );

          await ad4mClient.runtime.addTrustedAgents([
            (await bobAd4mClient.agent.me()).did,
          ]);

          const installGetLanguage = await ad4mClient.languages.byAddress(
            applyTemplateFromSource.address,
          );
          expect(installGetLanguage.address).to.be.equal(
            applyTemplateFromSource.address,
          );
        ...
        it("Bob can install Alice's perspective-diff-sync", async () => {
          ...
          const installSourceTrusted = await bobAd4mClient.languages.byAddress(
            sourceLanguage.address,
          );
          expect(installSourceTrusted.address).to.be.equal(
            sourceLanguage.address,
          );
        ...
        it("Bob can install his own templated language", async () => {
          ...
          const installTemplated = await bobAd4mClient.languages.byAddress(
            applyTemplateFromSource.address,
          );
          expect(installTemplated.address).to.be.equal(
            applyTemplateFromSource.address,
          );
          expect(installTemplated.name).to.be.equal(
            "Bob's templated perspective-diff-sync",
          );
        ...
        it("Bob can delete a language", async () => {
          const deleteLanguage = await bobAd4mClient.languages.remove(
            sourceLanguage.address,
          );

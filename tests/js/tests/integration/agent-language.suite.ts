    ...
    it("works across remote agents", async () => {
      ...
      async function retryAgentLookup(
        ...
        while (!result && attempts < maxAttempts) {
          if (attempts % 10 === 0) {
            console.log(
              `${clientName} looking up ${targetName}... attempt ${attempts}/${maxAttempts}`,
            );
          }
          await sleep(1000);
        ...
        if (!result) {
          console.error(
            `${clientName} failed to find ${targetName} after ${maxAttempts} attempts`,
          );
      ...
      const bobSeenFromAlice = await retryAgentLookup(
        alice,
        didBob,
        "Alice",
        "Bob",
      ...
      expect(
        bobSeenFromAlice,
        "Alice should be able to see Bob's agent profile",
      ).to.not.be.null;
      expect(bobSeenFromAlice).to.be.eql(bobHimself);

      const aliceSeenFromBob = await retryAgentLookup(
        bob,
        didAlice,
        "Bob",
        "Alice",
      ...
      expect(
        aliceSeenFromBob,
        "Bob should be able to see Alice's agent profile",
      ).to.not.be.null;

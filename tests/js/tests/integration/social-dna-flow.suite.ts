import { Link, LinkQuery, Literal, SHACLFlow } from "@coasys/ad4m";
  ...
  return () => {
    describe("There is a SDNA test exercising an example TODO SDNA", () => {
      ...
      it.skip("can add social DNA to perspective and go through flow", async () => {
        ...
        let sDNAFacts = await ad4mClient!.perspective.queryLinks(
          perspective.uuid,
          new LinkQuery({
            source: "ad4m://self",
        ...
        await perspective.add(
          new Link({ source: "ad4m://self", target: "test-lang://1234" }),
        );
        let availableFlows =
          await perspective.availableFlows("test-lang://1234");
        expect(availableFlows.length).to.be.equal(1);
        expect(availableFlows[0]).to.be.equal("TODO");
        let startAction = await perspective.infer(
          `start_action(Action, F), register_sdna_flow("TODO", F)`,
        );
        ...
        let expressionsInTodo = await perspective.expressionsInFlowState(
          "TODO",
          0,
        );
        ...
        // continue flow
        let flowActions = await perspective.flowActions(
          "TODO",
          "test-lang://1234",
        );
        ...
        expressionsInTodo = await perspective.expressionsInFlowState(
          "TODO",
          0.5,
        );
        expect(expressionsInTodo.length).to.be.equal(1);

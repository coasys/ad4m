...
export default function runtimeTests(testContext: TestContext) {
  return () => {
    it("Trusted Agents CRUD", async () => {
      ...
      expect(initalAgents).to.eql(
        [did, PERSPECT3VISM_AGENT, PUBLISHING_AGENT].sort(),
      );

      const addAgents = await ad4mClient.runtime.addTrustedAgents([
        "agentPubKey",
        "agentPubKey2",
      ]);
      expect(addAgents).to.eql(
        [
          did,
          PERSPECT3VISM_AGENT,
          PUBLISHING_AGENT,
      ...
      const addAgentsDuplicate = await ad4mClient.runtime.addTrustedAgents([
        "agentPubKey",
        "agentPubKey2",
      ]);
      expect(addAgentsDuplicate).to.eql(
        [
          did,
          PERSPECT3VISM_AGENT,
          PUBLISHING_AGENT,
      ...
      const deleteAgents1 = await ad4mClient.runtime.deleteTrustedAgents([
        "agentPubKey2",
      ]);
      expect(deleteAgents1).to.eql(
        [did, PERSPECT3VISM_AGENT, PUBLISHING_AGENT, "agentPubKey"].sort(),
      );

      const deleteAgents2 = await ad4mClient.runtime.deleteTrustedAgents([
        "agentPubKey",
        "agentPubKey2",
      ]);
      expect(deleteAgents2).to.eql(
        [did, PERSPECT3VISM_AGENT, PUBLISHING_AGENT].sort(),
      );

      const getAgentsPostDelete = await ad4mClient.runtime.getTrustedAgents();
      expect(getAgentsPostDelete).to.eql(
        [did, PERSPECT3VISM_AGENT, PUBLISHING_AGENT].sort(),
      );
    });
    ...
    it("CRUD for known LinkLanguage templates", async () => {
      ...
      const addAddresses =
        await ad4mClient.runtime.addKnownLinkLanguageTemplates([
          "Qm123",
          "Qmabc",
        ]);
      expect(addAddresses).to.eql(
        [DIFF_SYNC_OFFICIAL, "Qm123", "Qmabc"].sort(),
      );
      ...
      const deleted = await ad4mClient.runtime.removeKnownLinkLanguageTemplates(
        ["Qm123"],
      );
      expect(deleted).to.eql([DIFF_SYNC_OFFICIAL, "Qmabc"].sort());

      const deleted2 =
        await ad4mClient.runtime.removeKnownLinkLanguageTemplates([
          "Qm123",
          "Qmabc",
        ]);
      ...
      const getPostDelete =
        await ad4mClient.runtime.knownLinkLanguageTemplates();
    ...
    it("CRUD for friends", async () => {
      ...
      const added = await ad4mClient.runtime.addFriends([
        "did:test:1",
        "did:test:2",
      ]);
      ...
      //Add the agents again to be sure we cannot get any duplicates
      const addDuplicate = await ad4mClient.runtime.addFriends([
        "did:test:1",
        "did:test:2",
      ]);
      ...
      const deleted2 = await ad4mClient.runtime.removeFriends([
        "did:test:1",
        "did:test:2",
      ]);
    ...
    it("doesn't mix up stores", async () => {
      ...
      const deleted = await ad4mClient.runtime.removeFriends([
        "did:test:1",
        "agentPubKey",
        "Qm123",
      ]);
      ...
      const postDeleteAddresses =
        await ad4mClient.runtime.knownLinkLanguageTemplates();
      expect(postDeleteAddresses).to.eql(
        [DIFF_SYNC_OFFICIAL, "Qm123", "Qmabc"].sort(),
      );

      const postDeleteAgents = await ad4mClient.runtime.getTrustedAgents();
      expect(postDeleteAgents).to.eql(
        [
          did,
          PERSPECT3VISM_AGENT,
          PUBLISHING_AGENT,
    ...
    it("can get runtimeInfo", async () => {
      ...
      expect(runtimeInfo.ad4mExecutorVersion).to.be.equal(
    ...
    it("can handle notifications", async () => {
      ...
      mockFunction.callsFake((exception) => {
        ...
        if (exception.type === ExceptionType.InstallNotificationRequest) {
          ...
          if (requestedNotification.description === notification.description) {
            expect(requestedNotification.appName).to.equal(
              notification.appName,
            );
            expect(requestedNotification.appUrl).to.equal(notification.appUrl);
            expect(requestedNotification.appIconPath).to.equal(
              notification.appIconPath,
            );
            expect(requestedNotification.trigger).to.equal(
              notification.trigger,
            );
            expect(requestedNotification.perspectiveIds).to.eql(
              notification.perspectiveIds,
            );
            expect(requestedNotification.webhookUrl).to.equal(
              notification.webhookUrl,
            );
            expect(requestedNotification.webhookAuth).to.equal(
      ...
      const granted =
        await ad4mClient.runtime.grantNotification(notificationId);
      ...
      const updated = await ad4mClient.runtime.updateNotification(
        notificationId,
        updatedNotification,
      );
      ...
      const updatedNotificationInList = updatedNotificationCheck.find(
      ...
      expect(updatedNotificationInList?.description).to.equal(
        updatedNotification.description,
      );

      // Check if the notification is removed
      const removed =
        await ad4mClient.runtime.removeNotification(notificationId);
    ...
    it("can trigger notifications", async () => {
      ...
      let notificationPerspective = await ad4mClient.perspective.add(
        "notification test perspective",
      );
      let otherPerspective =
        await ad4mClient.perspective.add("other perspective");
      ...
      await notificationPerspective.add(
        new Link({ source: "control://source", target: "control://target" }),
      );
      await sleep(1000);
      expect(mockFunction.called).to.be.false;

      // Ensuring only selected perspectives will trigger
      await otherPerspective.add(
        new Link({
      ...
      let triggeredNotification = mockFunction.getCall(0)
        .args[0] as TriggeredNotification;
      expect(triggeredNotification.notification.description).to.equal(
      ...
      triggeredNotification = mockFunction.getCall(1)
        .args[0] as TriggeredNotification;
    ...
    it("can detect mentions in notifications (Flux example)", async () => {
      ...
      let notificationPerspective =
        await ad4mClient.perspective.add("flux mention test");
      ...
      //@ts-ignore
      expect(triggerMatch[0].perspective_id).to.equal(
        notificationPerspective.uuid,
      );
    });
    ...
    it("can export and import database", async () => {
      ...
      // Clear some data
      await ad4mClient.runtime.removeFriends([
        "test-friend-1",
        "test-friend-2",
      ]);
      await ad4mClient.runtime.deleteTrustedAgents([
        "test-agent-1",
        "test-agent-2",
      ]);
      ...
      expect(trustedAgents).to.include.members([
        "test-agent-1",
        "test-agent-2",
      ]);

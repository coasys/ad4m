  ...
  return () => {
    describe("Neighbourhood", () => {
      it("can publish and join locally @alice", async () => {
        ...
        const socialContext =
          await ad4mClient.languages.applyTemplateAndPublish(
            ...
            JSON.stringify({
              uid: uuidv4(),
              name: "Alice's perspective-diff-sync",
            }),
          );
        ...
        expect(perspective?.neighbourhood!.data.linkLanguage).to.be.equal(
          socialContext.address,
        );
        expect(perspective?.neighbourhood!.data.meta.links.length).to.be.equal(
          1,
        );

        // The perspective should start in NeighbourhoodCreationInitiated state
        expect(perspective?.state).to.be.equal(
        ...
        while (
          currentPerspective?.state !== PerspectiveState.Synced &&
          tries < maxTries
        ) {
          await sleep(1000);
      ...
      it("can be created by Alice and joined by Bob", async () => {
        ...
        expect(socialContext.name).to.be.equal(
          "Alice's neighbourhood with Bob",
        );
        const neighbourhoodUrl =
          await alice.neighbourhood.publishFromPerspective(
            aliceP1.uuid,
            socialContext.address,
            new Perspective(),
          );
      ...
      it("shared link created by Alice received by Bob", async () => {
        ...
        const socialContext = await alice.languages.applyTemplateAndPublish(
          ...
          JSON.stringify({
            uid: uuidv4(),
            name: "Alice's neighbourhood with Bob test shared links",
          }),
        );
        const neighbourhoodUrl =
          await alice.neighbourhood.publishFromPerspective(
        ...
        await testContext.makeAllNodesKnown();
        expect(bobP1!.state).to.be.oneOf([
      ...
      it("local link created by Alice NOT received by Bob", async () => {
        ...
        const socialContext = await alice.languages.applyTemplateAndPublish(
          ...
          JSON.stringify({
            uid: uuidv4(),
            name: "Alice's neighbourhood with Bob test local links",
          }),
        );
        const neighbourhoodUrl =
          await alice.neighbourhood.publishFromPerspective(
        ...
        await alice.perspective.addLink(
          aliceP1.uuid,
          { source: "ad4m://root", target: "test://test" },
          "local",
        );
      ...
      it("stress test - Bob receives 1500 links created rapidly by Alice", async () => {
        ...
        const socialContext = await alice.languages.applyTemplateAndPublish(
          ...
          JSON.stringify({
            uid: uuidv4(),
            name: "Alice's neighbourhood with Bob stress test",
          }),
        );
        const neighbourhoodUrl =
          await alice.neighbourhood.publishFromPerspective(
        ...
        for (let i = 0; i < 1500; i++) {
          ...
          const link = await alice.perspective.addLink(aliceP1.uuid, {
            source: "ad4m://root",
            target: `test://test/${i}`,
          });
        ...
        while (bobLinks.length < 1500 && tries < maxTries) {
          console.log(
            `Bob retrying getting links... Got ${bobLinks.length}/1500`,
          );
          await sleep(1000);
          bobLinks = await bob.perspective.queryLinks(
            bobP1!.uuid,
        ...
        expect(bobLinks.some((link) => link.data.target === "test://test/0")).to
          .be.true;
        expect(bobLinks.some((link) => link.data.target === "test://test/749"))
          .to.be.true;
        expect(bobLinks.some((link) => link.data.target === "test://test/1499"))
          .to.be.true;
        ...
        await testContext.alice.perspective.addLink(aliceP1.uuid, {
          source: "ad4m://alice",
          target: "test://alice/1",
        });
        await testContext.alice.perspective.addLink(aliceP1.uuid, {
        ...
        bobLinks = await testContext.bob.perspective.queryLinks(
          bobP1.uuid,
          new LinkQuery({ source: "ad4m://alice" }),
        );
        let bobTries = 1;
        ...
        while (bobLinks.length < 3 && bobTries < maxTriesBob) {
          console.log(
            `Bob retrying getting Alice's links... Got ${bobLinks.length}/3`,
          );
          await sleep(2000);
          bobLinks = await testContext.bob.perspective.queryLinks(
            bobP1.uuid,
        ...
        expect(bobLinks.some((link) => link.data.target === "test://alice/1"))
          .to.be.true;
        expect(bobLinks.some((link) => link.data.target === "test://alice/2"))
          .to.be.true;
        expect(bobLinks.some((link) => link.data.target === "test://alice/3"))
          .to.be.true;

        // Bob creates some links
        console.log("Bob creating links...");
        await testContext.bob.perspective.addLink(bobP1.uuid, {
          source: "ad4m://bob",
          target: "test://bob/1",
        });
        await testContext.bob.perspective.addLink(bobP1.uuid, {
        ...
        // Wait for sync with retry loop
        let aliceLinks = await testContext.alice.perspective.queryLinks(
          aliceP1.uuid,
          new LinkQuery({ source: "ad4m://bob" }),
        );
        tries = 1;
        ...
        while (aliceLinks.length < 3 && tries < maxTriesAlice) {
          console.log(
            `Alice retrying getting links... Got ${aliceLinks.length}/3`,
          );
          await sleep(2000);
          aliceLinks = await testContext.alice.perspective.queryLinks(
            aliceP1.uuid,
            new LinkQuery({ source: "ad4m://bob" }),
          );
          tries++;
        ...
        expect(aliceLinks.some((link) => link.data.target === "test://bob/1"))
          .to.be.true;
        expect(aliceLinks.some((link) => link.data.target === "test://bob/2"))
          .to.be.true;
        expect(aliceLinks.some((link) => link.data.target === "test://bob/3"))
          .to.be.true;
      });
      ...
      it("can delete neighbourhood", async () => {
        ...
        const deleteNeighbourhood = await alice.perspective.remove(
          aliceP1!.uuid,
        );
        expect(deleteNeighbourhood.perspectiveRemove).to.be.true;

        const bobDeleteNeighbourhood = await bob.perspective.remove(
          bobP1!.uuid,
        );
      ...
      describe("with set up and joined NH for Telepresence", async () => {
        ...
        before(async () => {
          ...
          const linkLang = await alice.languages.applyTemplateAndPublish(
            DIFF_SYNC_OFFICIAL,
            JSON.stringify({
              uid: uuidv4(),
              name: "Alice's neighbourhood for Telepresence",
            }),
          );
          const neighbourhoodUrl =
            await alice.neighbourhood.publishFromPerspective(
        ...
        it("they see each other in `otherAgents`", async () => {
          ...
          while (
            (aliceAgents.length < 1 || bobAgents.length < 1) &&
            tries < maxTries
          ) {
            console.log(
              `Waiting for agents to discover each other... Alice: ${aliceAgents.length}, Bob: ${bobAgents.length}`,
            );
            await sleep(1000);
        ...
        it("they can set their online status and see each others online status in `onlineAgents`", async () => {
          ...
          expect(aliceOnline[0].status.data.links).to.deep.equal(
            testPerspective.links,
          );

          expect(bobOnline.length).to.be.equal(1);
          expect(bobOnline[0].did).to.be.equal(aliceDID);
          expect(bobOnline[0].status.data.links).to.deep.equal(
            testPerspective.links,
          );

          await aliceNH!.setOnlineStatusU(
            PerspectiveUnsignedInput.fromLink(
              new Link({
                source: "test://source",
                target: "test://target",
              }),
          ...
          expect(bobOnline2[0].status.data.links[0].data.source).to.equal(
            "test://source",
          );
          expect(bobOnline2[0].status.data.links[0].data.target).to.equal(
            "test://target",
          );

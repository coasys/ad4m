...
describe("Email Verification with Mock Service", () => {
  ...
  describe("Email Verification Flow - New User Signup", () => {
    it("should complete full signup flow with email verification", async () => {
      ...
      console.log(
        `✅ Result: success=${verifyRequest.success}, requiresPassword=${verifyRequest.requiresPassword}, isExistingUser=${verifyRequest.isExistingUser}`,
      );
      console.log(`   Message: ${verifyRequest.message}`);

      console.log("\n🔹 Step 2: Create user (triggers verification email)");
      const createResult = await adminAd4mClient!.agent.createUser(
        email,
        password,
      );
      ...
      const token = await adminAd4mClient!.agent.verifyEmailCode(
        email,
        code!,
        "signup",
      );
    ...
    it("should enforce code expiration (15 minutes)", async () => {
      ...
      await adminAd4mClient!.agent.requestLoginVerification(email);
      const expiredCode =
        await adminAd4mClient!.runtime.emailTestGetCode(email);
      ...
      await adminAd4mClient!.runtime.emailTestSetExpiry(
        email,
        "login",
        pastTimestamp,
      );
      ...
      try {
        await adminAd4mClient!.agent.verifyEmailCode(
          email,
          expiredCode!,
          "login",
        );
  ...
  describe("Email Verification Flow - Existing User Login", () => {
    ...
    before(async () => {
      ...
      const signupCode =
        await adminAd4mClient!.runtime.emailTestGetCode(existingEmail);
      await adminAd4mClient!.agent.verifyEmailCode(
        existingEmail,
        signupCode!,
        "signup",
    ...
    it("should complete login flow with email verification (passwordless)", async () => {
      ...
      const verifyRequest =
        await adminAd4mClient!.agent.requestLoginVerification(existingEmail);
  ...
  describe("Mock Service Functionality", () => {
    it("should capture codes for multiple users simultaneously", async () => {
      ...
      // Retrieve all codes
      for (const user of users) {
        const code = await adminAd4mClient!.runtime.emailTestGetCode(
          user.email,
        );
        expect(code).to.exist;
        expect(code!.length).to.equal(6);
    ...
    describe("Failed Attempt Rate Limiting", () => {
      ...
      it("should invalidate code after 5 failed attempts", async () => {
        ...
        for (let i = 1; i <= 5; i++) {
          try {
            ...
            if (i < 5) {
              ...
              console.log(
                `✅ Code invalidated after ${i} failed attempts: ${e.message}`,
              );
            }
          }
        }
        ...
        try {
          ...
          console.log(
            `✅ Real code correctly rejected after invalidation: ${e.message}`,
          );
        }
      });
      ...
      it("should reset failed attempts counter on successful verification", async () => {
        ...
        console.log(
          "✅ Failed attempts counter reset after successful verification",
        );
      });
      ...
      it("should handle concurrent failed attempts correctly", async () => {
        ...
        const promises = Array.from({ length: 5 }, () =>
          adminAd4mClient!.agent
            .verifyEmailCode(email, "000000", "signup")
            .catch((e) => e),
        );
        ...
        expect(codeAfterInvalidation).to.be.null;
        console.log(
          "✅ Concurrent failed attempts correctly handled and code invalidated",
        );
      });
    });
  });

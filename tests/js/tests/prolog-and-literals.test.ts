      ...
      describe("Active record implementation", () => {
        ...
        it("find()", async () => {
          let recipe1 = new Recipe(
            perspective!,
            Literal.from("Active record implementation test find").toUrl(),
          );
        ...
        it("can constrain collection entries clause", async () => {
          let root = Literal.from(
            "Active record implementation collection test",
          ).toUrl();
          const recipe = new Recipe(perspective!, root);
        ...
        it("delete()", async () => {
          let recipe1 = new Recipe(
            perspective!,
            Literal.from("Active record implementation test delete1 ").toUrl(),
          );
          ...
          let recipe2 = new Recipe(
            perspective!,
            Literal.from("Active record implementation test delete2 ").toUrl(),
          );
          ...
          let recipe3 = new Recipe(
            perspective!,
            Literal.from("Active record implementation test delete3 ").toUrl(),
          );
        ...
        it("findAll() works with source prop", async () => {
          ...
          // @ts-ignore - 3-arg constructor (source) is a legacy API
          ...
          // @ts-ignore - source is a legacy Query param not yet in Query type
          const source1Recipes = await Recipe.findAll(perspective!, {
            source: source1,
          });
          ...
          // @ts-ignore - source is a legacy Query param not yet in Query type
          const source2Recipes = await Recipe.findAll(perspective!, {
            source: source2,
          });
        ...
        it("findAll() works with properties query", async () => {
          ...
          expect(recipesWithAllAttributes[0].name).to.equal(
            "recipe://test_name",
          );
          ...
          const recipesWithNameOnly = await Recipe.findAll(perspective!, {
            properties: ["name"],
          });
          ...
          const recipesWithTypeAndBooleanTestOnly = await Recipe.findAll(
            perspective!,
            { properties: ["name", "booleanTest"] },
          );
          expect(recipesWithTypeAndBooleanTestOnly[0].name).to.equal(
            "recipe://test_name",
          );
          expect(recipesWithTypeAndBooleanTestOnly[0].booleanTest).to.equal(
            true,
          );

          // Test recipes with author only
          const recipesWithAuthorOnly = await Recipe.findAll(perspective!, {
            properties: ["author"],
          });
        ...
        it("findAll() works with collections query", async () => {
          ...
          // @ts-ignore - collections is a legacy Query param
          const recipesWithCommentsOnly = await Recipe.findAll(perspective!, {
          ...
          // @ts-ignore - collections is a legacy Query param
          const recipesWithEntriesOnly = await Recipe.findAll(perspective!, {
            collections: ["entries"],
          });
        ...
        it("findAll() works with basic where queries", async () => {
          ...
          const recipes6 = await Recipe.findAll(perspective!, {
            where: { author: me.did },
          });
          expect(recipes6.length).to.equal(3);
          // Test where with invalid author
          const recipes7 = await Recipe.findAll(perspective!, {
            where: { author: "This author doesn't exist" },
          });
          ...
          const recipes8 = await Recipe.findAll(perspective!, {
            where: { timestamp: validTimestamp1 },
          });
          expect(recipes8.length).to.equal(1);
          // Test where with invalid timestamp
          const recipes9 = await Recipe.findAll(perspective!, {
            where: { timestamp: invalidTimestamp },
          });
          expect(recipes9.length).to.equal(0);
          // Test where with an array of possible timestamp matches
          const recipes10 = await Recipe.findAll(perspective!, {
            where: { timestamp: [validTimestamp1, validTimestamp2] },
          });
        ...
        it("findAll() works with where query not operations", async () => {
          ...
          const me = await ad4m!.agent.me();
          const recipes2 = await Recipe.findAll(perspective!, {
            where: { author: { not: me.did } },
          });
          expect(recipes2.length).to.equal(0);

          // Test not operation on timestamp
          const recipes3 = await Recipe.findAll(perspective!, {
            where: { timestamp: { not: validTimestamp1 } },
          });
          expect(recipes3.length).to.equal(2);

          // Test not operation with an array of possible string matches
          const recipes4 = await Recipe.findAll(perspective!, {
            where: { name: { not: ["Recipe 1", "Recipe 2"] } },
          });
          expect(recipes4.length).to.equal(1);
          ...
          const recipes5 = await Recipe.findAll(perspective!, {
            where: { timestamp: { not: [validTimestamp1, validTimestamp2] } },
          });
        ...
        it("findAll() works with where query lt, lte, gt, & gte operations", async () => {
          ...
          const recipe2timestamp =
            typeof sortedRecipes[1].timestamp === "number"
        ...
        it("findAll() works with where query between operations", async () => {
          ...
          const lowToMediumTasks = await TaskDue.findAll(perspective!, {
          ...
          const mediumToHighTasks = await TaskDue.findAll(perspective!, {
            where: { priority: { between: [5, 10] } },
          });
          expect(mediumToHighTasks.length).to.equal(2);

          // Test between operation on dueDate
          const earlyTasks = await TaskDue.findAll(perspective!, {
            where: { dueDate: { between: [start, mid] } },
          });
          expect(earlyTasks.length).to.equal(1);

          // Test between operation on dueDate with different values
          const laterTasks = await TaskDue.findAll(perspective!, {
            where: { dueDate: { between: [mid, end] } },
          });
        ...
        it("findAllAndCount() returns both the retrived instances and the total count", async () => {
          ...
          const { results: recipes1, totalCount: count1 } =
            await Recipe.findAllAndCount(perspective!, {
              limit: 2,
              count: true,
            });
          expect(recipes1.length).to.equal(2);
          ...
          const { results: recipes3, totalCount: count3 } =
            await Recipe.findAllAndCount(perspective!, {
              offset: 3,
              limit: 3,
              count: true,
          ...
          const { results: recipes2, totalCount: count2 } =
            await Recipe.findAllAndCount(perspective!, {
              where: { name: ["Recipe 1", "Recipe 2", "Recipe 3"] },
              limit: 2,
              count: true,
            });
          expect(recipes2.length).to.equal(2);
          ...
          const { results: recipes4, totalCount: count4 } =
            await Recipe.findAllAndCount(perspective!, {
              where: { number: 5 },
              offset: 3,
              limit: 3,
          ...
          const { results: recipes5, totalCount: count5 } =
            await Recipe.findAllAndCount(perspective!, {
              where: { number: 3 },
              offset: 3,
              limit: 3,
          ...
          const { results: recipes6, totalCount: count6 } =
            await Recipe.findAllAndCount(perspective!, {
              where: { name: { not: "Recipe 1" } },
              limit: 3,
              count: true,
          ...
          const { results: recipes7, totalCount: count7 } =
            await Recipe.findAllAndCount(perspective!, {
              where: { name: { not: "Recipe 2" } },
              offset: 1,
              limit: 3,
              count: true,
            });
          ...
          const { results: recipes8, totalCount: count8 } =
            await Recipe.findAllAndCount(perspective!, {
              where: { name: { not: "Recipe 4" } },
              offset: 3,
              limit: 3,
              count: true,
            });
        ...
        it("paginate() helper function works with pageNumber & pageSize props", async () => {
          ...
          const { results: recipes1, totalCount: count1 } =
            await Recipe.paginate(perspective!, 2, 1);
          expect(recipes1.length).to.equal(2);
          expect(count1).to.equal(6);
          expect(recipes1[0].name).to.equal("Recipe 1");
          ...
          const { results: recipes2, totalCount: count2 } =
            await Recipe.paginate(perspective!, 3, 2, {
              where: { name: { not: "Recipe 4" } },
            });
          expect(recipes2.length).to.equal(2);
        ...
        it("count() returns only the count without retrieving instances", async () => {
          ...
          const count2 = await Recipe.count(perspective!, {
            where: { name: ["Recipe 1", "Recipe 2", "Recipe 3"] },
          });
          expect(count2).to.equal(3);

          // Test count with more complex constraints
          const count3 = await Recipe.count(perspective!, {
            where: { name: { not: "Recipe 1" } },
          });
        ...
        it("count() and countSubscribe() work on the query builder", async () => {
          ...
          const filteredQuery = Recipe.query(perspective!).where({
          ...
          const countSub = builder.live((results) => {
            lastCount = results.length;
          });
          const subscription = await builder.count();
          ...
          await waitForCondition(() => lastCount === 4, {
            timeoutMs: 15000,
        ...
        it("count() works with advanced where conditions (gt, between, timestamp)", async () => {
          ...
          const countGt3 = await Recipe.count(perspective!, {
            where: { number: { gt: 3 } },
          });
          const findAllGt3 = await Recipe.findAll(perspective!, {
            where: { number: { gt: 3 } },
          });
          ...
          const countBetween2And4 = await Recipe.count(perspective!, {
            where: { number: { between: [2, 4] } },
          });
          const findAllBetween2And4 = await Recipe.findAll(perspective!, {
            where: { number: { between: [2, 4] } },
          });
          ...
          const countGte2Lte4 = await Recipe.count(perspective!, {
            where: { number: { gte: 2, lte: 4 } },
          });
          const findAllGte2Lte4 = await Recipe.findAll(perspective!, {
            where: { number: { gte: 2, lte: 4 } },
          });
          ...
          const countLt3 = await Recipe.count(perspective!, {
            where: { number: { lt: 3 } },
          });
          const findAllLt3 = await Recipe.findAll(perspective!, {
            where: { number: { lt: 3 } },
          });
          ...
          const allRecipes = await Recipe.findAll(perspective!);
          const recipe3Instance = allRecipes.find(
            (r: any) => r.name === "Recipe 3",
          );
          expect(recipe3Instance).to.not.be.undefined;

          if (recipe3Instance && recipe3Instance.timestamp) {
            // Convert timestamp to number if it's a string
            const recipe3Timestamp =
              typeof recipe3Instance.timestamp === "string"
        ...
        it("paginate() and paginateSubscribe() work on the query builder", async () => {
          ...
          const paginateSub = query.live(async (r) => {
            // re-paginate on every update to mimic paginateSubscribe
            lastResult = await query.paginate(3, 1);
          });
          const initialResult = await query.paginate(3, 1);
          ...
          // Use longer timeout for CI environments which may be slower
          await waitForCondition(() => lastResult !== null, {
            timeoutMs: 15000,
            errorMessage:
              "Paginate subscription did not update after recipe save",
          });
        ...
        it("query builder works with subscriptions", async () => {
          ...
          let updateCount = 0;
          const builder = Notification.query(perspective!).where({
            priority: { gt: 5 },
            read: false,
          });
          const notifSub = builder.live((newNotifications) => {
        ...
        it("query builder should filter by subject class", async () => {
          ...
          const note1Results = await Note1.query(perspective!)
        ...
        it("query builder works with single query object, complex query and subscriptions", async () => {
          ...
          const taskSub = builder.live((newTasks) => {
            tasks = newTasks;
            updateCount++;
          });
          const initialResults = await builder.get();
        ...
        describe("ModelQueryBuilder", () => {
          ...
          it("handles subscriptions and disposal correctly", async () => {
            ...
            const builder = TestModel.query(perspective).where({
              status: "active",
            });
            ...
            const sub1 = builder.live(callback1);
            const initialResults1 = await builder.get();
            ...
            // Wait for subscription update with proper condition checking
            await waitForCondition(() => callback1.called, {
              timeoutMs: 5000,
            ...
            // Create second subscription (dispose first one)
            sub1.unsubscribe();
            const sub2 = builder.live(callback2);
            const initialResults2 = await builder.get();
            ...
            // Wait for subscription update with proper condition checking
            await waitForCondition(() => callback2.called, {
              timeoutMs: 5000,
          ...
          it("handles count subscriptions and disposal", async () => {
            ...
            const countSub2 = builder.live((results) =>
            ...
            // Wait for subscription update with proper condition checking
            // Use longer timeout for CI environments which may be slower
            await waitForCondition(() => countCallback.called, {
              timeoutMs: 15000,
          ...
          it("handles paginated subscriptions and disposal", async () => {
            ...
            const pageSub = builder.live(async (r) => {
              pageCallback(await builder.paginate(2, 1));
            });
            const initialPage = await builder.paginate(2, 1);
            ...
            await waitForCondition(
                ...
                pageCallback.lastCall.args[0].results.length >= 2,
              {
                timeoutMs: 15000,
                errorMessage:
                  "Paginate callback was not called with expected results after model saves",
              },
            );
  ...
  describe("Ad4mModel.fromJSONSchema", () => {
    ...
    describe("with explicit configuration", () => {
      it("should create Ad4mModel class from JSON Schema with explicit namespace", async () => {
        ...
        // @ts-ignore - baseExpression is the legacy name for .id

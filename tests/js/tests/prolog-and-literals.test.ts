...
describe("Prolog + Literals", () => {
    ...
    describe("SHACL RPC endpoints (perspective.getShacl*)", () => {
        // These four endpoints move shape resolution server-side (see PR #935):
        // one round trip each instead of the multi-step queryLinks walk the SDK
        // used to perform client-side. Own perspective (not the shared one above)
        // so the "before any class is registered" assertions aren't polluted by
        // the SDNA classes registered earlier in this file.
        let shaclPerspective: PerspectiveProxy | null = null
        ...
        before(async () => {
            shaclPerspective = await ad4m!.perspective.add("shacl-rpc-endpoints")
        })

        after(async () => {
            if (shaclPerspective) {
                await ad4m!.perspective.remove(shaclPerspective.uuid)
            }
        })

        describe("before any SHACL class is registered", () => {
            it("getShaclNames() returns []", async () => {
                expect(await shaclPerspective!.getShaclNames()).to.deep.equal([])
            })

            it("getAllShacl() returns []", async () => {
                expect(await shaclPerspective!.getAllShacl()).to.deep.equal([])
            })

            it("getShaclTargetClass('Todo') returns undefined", async () => {
                expect(await shaclPerspective!.getShaclTargetClass("Todo")).to.be.undefined
            })

            it("getShacl('Todo') returns null", async () => {
                expect(await shaclPerspective!.getShacl("Todo")).to.be.null
            })
        })

        describe("after registering Message + Todo", () => {
            before(async () => {
                await shaclPerspective!.ensureSDNASubjectClass(ShaclMessage)
                await shaclPerspective!.ensureSDNASubjectClass(ShaclTodo)
            })

            it("getShaclNames() returns the two registered names", async () => {
                const names = await shaclPerspective!.getShaclNames()
            ...
            it("getShaclTargetClass() resolves each registered name to the class URI", async () => {
                ...
                expect(await shaclPerspective!.getShaclTargetClass("Message")).to.equal("ad4m://Message")
                expect(await shaclPerspective!.getShaclTargetClass("Todo")).to.equal("todo://Todo")
            })

            it("getShaclTargetClass() returns undefined for an unknown name", async () => {
                expect(await shaclPerspective!.getShaclTargetClass("DoesNotExist")).to.be.undefined
            })

            it("getShacl('Todo') returns a SHACLShape with the expected shape URI + target class", async () => {
                const shape = await shaclPerspective!.getShacl("Todo")
            ...
            it("getShacl('Message') reconstructs a shape with the Flag + Optional properties", async () => {
                const shape = await shaclPerspective!.getShacl("Message")
                expect(shape).to.not.be.null
            ...
            it("getShacl() returns null for an unknown name", async () => {
                expect(await shaclPerspective!.getShacl("DoesNotExist")).to.be.null
            })

            it("getAllShacl() returns both shapes with matching name+targetClass+property counts", async () => {
                const all = await shaclPerspective!.getAllShacl()
                expect(all.length).to.equal(2)
            ...
            it("getShacl(name).properties matches the single-shape RPC result inside getAllShacl()", async () => {
                ...
                const single = await shaclPerspective!.getShacl("Todo")
                const all = await shaclPerspective!.getAllShacl()

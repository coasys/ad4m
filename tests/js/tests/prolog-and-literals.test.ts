    ...
    describe("Subjects", () => {
        ...
        describe("SDNA creation decorators", () => {
            ...
            describe("with Vector Embedding", () => {
              ...
              before(async () => {
                // @ts-ignore
                const { name, sdna } = Embedding.generateSDNA();
                console.log("EMBEDDING:", name, sdna)
                await perspective!.addSdna(name, sdna, "subject_class");
              });
      
              it("save() & get()", async () => {
                console.log("save&get 1")

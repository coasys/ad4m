import type { LanguageExpression, EntryHash } from "./types";
...
export class LanguageStorage {
  ...
  async storeLanguageExpression(fileExpression: LanguageExpression): Promise<EntryHash> {
  ...
  async fetchChunk(fileChunkHash: EntryHash): Promise<Blob> {
    ...
    while (bytes === null && tries < 10) {
      tries++
      bytes = await this.#zomeCall("get_language_chunk", fileChunkHash);
      if (bytes === null) {

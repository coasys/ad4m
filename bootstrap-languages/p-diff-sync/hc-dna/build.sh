...
echo "p-diff-sync build.sh: using hc=$HC ($("$HC" --version))"

CARGO_TARGET_DIR=target RUSTFLAGS='--cfg getrandom_backend="custom"' cargo build --release --target wasm32-unknown-unknown -p perspective_diff_sync -p perspective_diff_sync_integrity
"$HC" dna pack workdir
"$HC" app pack workdir
...
impl Workspace {
    ...
    fn handle_parents<Retriever: PerspectiveDiffRetreiver>(
        ...
        let resolved_diff = if current_diff.is_chunked() {
            debug!(
                "===Workspace.handle_parents(): Entry {:?} is CHUNKED - loading {} chunk(s)",
                current_hash,
                current_diff.diff_chunks.as_ref().unwrap().len()
            );
            let loaded_diff = load_diff_from_entry::<Retriever>(&current_diff)?;
            debug!(
                "===Workspace.handle_parents(): Loaded chunked diff - additions: {}, removals: {}",
                loaded_diff.additions.len(),
                loaded_diff.removals.len()
            );

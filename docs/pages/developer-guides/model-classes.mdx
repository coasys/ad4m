...
interface JSONSchemaToModelOptions {
  ...
  namespace?: string;         // Base namespace for predicates (e.g., "recipe://")
  ...
  // Property-specific overrides (per-property decorator options)
  propertyOptions?: Record<string, {
    ...
    writable?: boolean;       // Override for this property (default true)
    initial?: any;            // Initial value (generates constructor action)
  }>;
...
2. **`options.propertyOptions[name].through`** (per-property override)
3. **`x-ad4m.through` in property schema**
4. **`predicateTemplate`** (supports `${scheme}`, `${namespace}`/`${ns}`, `${title}`, `${property}`)
5. **`predicateGenerator(title, property)`**
6. **Default**: `${namespace}/${propertyName}` (normalized)

...
The second parameter to `fromJSONSchema()` accepts these options:

```typescript
interface JSONSchemaToModelOptions {
  // Model configuration
  name?: string;              // Override class name (defaults to schema.title)
  namespace?: string;         // Base namespace for predicates
  ...
  writable?: boolean;        // Whether properties are writable
  
  // Property-specific overrides
  properties?: {
    [propertyName: string]: {
      through?: string;         // Custom predicate URI
      resolveLanguage?: string; // Override for this property
      local?: boolean;         // Override for this property
      writable?: boolean;      // Override for this property
      initial?: any;           // Initial value
    }
  };
  
  // Namespace/predicate generation
  namespaceTemplate?: string; // Template for generating namespaces
  predicateTemplate?: string; // Template for generating predicates
  predicateGenerator?: (propertyName: string, schema: any) => string;
}
```
...
Each property's predicate URI is determined by:

1. **`options.properties[name].through`** (explicit override)
2. **`x-ad4m.through` in property schema**
3. **`predicateTemplate`** (e.g., `"{namespace}{property}"`)
4. **`predicateGenerator` function**
5. **Default**: `{namespace}{propertyName}`
...
const Product = Ad4mModel.fromJSONSchema(schema, {
  ...
  properties: {
    name: {
      through: "product://title"  // Override via options
    }
  }
});
```
...
const User = Ad4mModel.fromJSONSchema(advancedSchema);
```
...
post.addTag("blockchain");
post.removeCategory("technology");
```

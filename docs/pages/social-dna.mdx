...
Let's say we want to define `top_comments` as any expression in the Perspective with **more than 5 likes**:

<Callout type="info" emoji="ℹ️">
  Don't worry if this looks completely crazy right now. You will normally either
  use AI to write it for you, or you will use some of our abstraction layers to
  avoid the pain of writing Prolog code yourself.
</Callout>

```prolog filename="top_comments.pl"
top_comments(Source, Target, Author, Timestamp) :-
    link(Source, "sioc:/likes", Target, Author, Timestamp),
    count_likes(Target, Count),
    Count > 5.

count_likes(Target, Count) :-
    findall(_, link(_, "sioc:/likes", Target, _, _), Likes),
    length(Likes, Count).
```

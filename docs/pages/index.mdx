...
Built on Holochain, AD4M forms an agent-centric spanning layer atop the internet, harmonizing fragmented ecosystems into a distributed 
...
const perspective = await ad4mClient.perspective.byUUID("2f168edc-...");
console.log(perspective.links); // RDF triples: [{source: "me", predicate: "posted", target: "QmXyz123://post456"}]
```
...
Out of the gate, AD4M relies on bootstrap Languages—pre-installed essentials like agent, perspective, language-language. 
...
it's users to spawn their own ontologies and spaces into this meta-space.
...
```javascript
const newLanguage = await ad4mClient.expression.get("language://QmCustom321");
await ad4mClient.languages.install(newLanguage);
console.log(await ad4mClient.languages.all()); // Includes "my-social-app" at QmCustom321
```
...
Since every Expression contains it's crytographic provenance, AD4M has a natural built-in way to apply code-signing and verification-only Languages of trusted agents
...
interface CustomLanguage {
  ...
  name: "my-social-app";
  expressionTypes: {
    create: (content: { text: string }) => "QmCustom321://post789";
    get: (address: string) => { text: string };
  };
}
```
...
Main difference to the semantic web style graphs is that each triple (called `Link` in AD4M) is an Expression and thus has a cryptographic provenance.
These triples, like `<subject> <predicate> <object>`, store data such as `<agent123> <posted> <QmXyz123://post456>`. 

Agents share thise local graph databases in a collaberative way with other AD4M agenst via Neighbourhoods, app-independent social contexts that act as shared, 
...
\a community’s social structure—its links and relationships—moves seamlessly across apps. For instance:
```javascript
...
const posts = await ad4mClient.perspective.queryProlog("memberPost(Post, Agent, 'neighbourhood://QmGroup789')");
...
import { Ad4mModel, Link, property, link } from "@coasys/ad4m";
...
class Post {
  ...
  @link("postedBy", "Agent")
  postedBy: string; // Agent DID

  @link("inNeighbourhood", "Neighbourhood")
  neighbourhood: string; // Neighbourhood UUID or address

  // Optional: Custom method leveraging Social DNA
  async isMemberPost(): Promise<boolean> {
    const query = `memberPost("${this.id}", "${this.postedBy}", "${this.neighbourhood}")`;
    const results = await this.queryProlog(query);
...
// Usage in an app
const post = new Post();
post.content = "Hello, group!";
post.postedBy = "did:key:z6Mk...";
post.neighbourhood = "neighbourhood://QmGroup789";
await post.save(); // Persists to Perspective, generates Prolog

const isValid = await post.isMemberPost();
console.log(isValid); // True if postedBy is a member
```

Behind this, the @Ad4mModel, @property, and @link decorators translate the class into Prolog rules like:
```prolog

% Auto-generated by decorators
post(Id, Content) :- link(Id, "content", Content).
postedBy(Post, Agent) :- link(Post, "postedBy", Agent).
inNeighbourhood(Post, Neighbourhood) :- link(Post, "inNeighbourhood", Neighbourhood).
member(Agent, Neighbourhood) :- link(Agent, "memberOf", Neighbourhood).
memberPost(Post, Agent, Neighbourhood) :-
    postedBy(Post, Agent),
    inNeighbourhood(Post, Neighbourhood),
    member(Agent, Neighbourhood).
```

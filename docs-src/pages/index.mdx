...
running on your device (or a trusted host) and exposing a WebSocket RPC API and an [MCP server](/developer-guides/mcp), for apps and AI agents alike to query and interact with.
For example, a chat app might connect to `http://localhost:12000` to fetch messages stored in a Perspective, addressed via a Language like
...
The local AD4M node stores, manages and provides access to an arbitrary number of Perspectives-subjective knowledge graphs of RDF triples.
...
Agents share these local graph databases in a collaborative way with other AD4M agents via Neighbourhoods, app-independent social contexts that act as shared,
...
AD4M provides a high-performance SPARQL query engine for working with graph data in Perspectives:

#### SPARQL: Fast Graph Queries

AD4M includes a SPARQL engine powered by Oxigraph, executing entirely server-side in Rust. SPARQL excels at:
...
```javascript
// Graph query - find who Alice follows
...
// Find posts by Alice
const posts = await perspective.querySparql(`
  SELECT ?target WHERE {
    ?link a rdf:Statement ;
          rdf:subject <user://alice> ;
...
**Important**: `Ad4mModel` (described below) uses the SPARQL engine internally for all operations, providing efficient queries with no code changes required.

For detailed examples and graph traversal patterns, see the [SPARQL Queries Guide](/developer-guides/sparql-queries).

#### Prolog: Logic Programming (Backward Compatibility)

For backward compatibility and advanced logic programming, AD4M also includes Prolog, optimized for reasoning over relational data like the RDF triples in Perspectives.
...
This TypeScript base class lets you define model classes with properties and relationships, with all queries executing server-side in Rust via SPARQL
for fast query performance. Here's how you define a model:
```typescript
...
class Todo extends Ad4mModel {
    ...
    @ReadOnly({
        ...
        getter: `SELECT (COUNT(?reaction) > 0 AS ?value) WHERE { <Base> <flux://has_reaction> ?reaction . FILTER(?reaction = <flux://thumbsup>) }`
    })
...
- Queries execute server-side in Rust via SPARQL
- Generates efficient queries automatically
- Stores data as graph links in the perspective
- Provides type-safe TypeScript interfaces

For example, `Todo.findAll(perspective, { where: { state: "todo://done" } })` automatically generates and executes an optimized SPARQL query.

Prolog is still supported for backward compatibility. See the [Model Classes Guide](/developer-guides/model-classes) and [SPARQL Queries Guide](/developer-guides/sparql-queries) for details.
...
Developers define their app's data model in familiar TypeScript, and AD4M handles the Social DNA—stored as SHACL shapes in a Perspective or Neighbourhood—making it both accessible and powerful. This abstraction lets you focus on app logic while the SPARQL engine provides fast server-side queries and SHACL schemas enable structured, validated data that scales across the network.

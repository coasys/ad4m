...
running on your device (or a trusted host) and exposing a GraphQL API and an [MCP server](/developer-guides/mcp), for apps and AI agents alike to query and interact with.
...
AD4M now includes **SurrealDB**, a modern database that provides **10-100x faster performance** than Prolog for most queries. SurrealDB uses SQL-like syntax (SurrealQL) and excels at:
...
For detailed examples and graph traversal patterns, see the [SurrealDB Queries Guide](/developer-guides/surreal-queries).
...
For backward compatibility, Prolog is still supported (pass `useSurrealDB: false` to query methods). See the [Model Classes Guide](/developer-guides/model-classes) and [SurrealDB Queries Guide](/developer-guides/surreal-queries) for details.

...
The recommended way to use batch operations is `Ad4mModel.transaction()`, which handles batch creation, commit, and error rollback automatically:
...
If any operation inside the callback throws, nothing is persisted — you don't need explicit try/catch for rollback.
...
1. **Prefer `transaction()`**: Use `Ad4mModel.transaction()` over manual `createBatch()`/`commitBatch()` — it handles commit and rollback automatically.

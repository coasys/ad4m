...
### High-Level: Ad4mModel.transaction()

The simplest way to use batch operations with models is the `transaction()` static method, which handles batch creation and commit automatically:
...
try {
  ...
  await existingRecipe.save(batchId);

...
SurrealDB stores links as graph edges with indexed source and target node references, enabling efficient multi-hop traversal. See the [Quick Reference](#quick-reference-graph-traversal-syntax) above for the full syntax.
...
`);
// Deduplicate in JS: [...new Set(friendsOfFriends.map(f => f.friend_of_friend))]

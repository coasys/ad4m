...
<Callout type="info">
**What is AD4M?** AD4M is an agent-centric spanning layer built on top of the internet, bridging fragmented ecosystems (P2P, federated, centralized) into a unified semantic layer. Each user runs their own executor (local runtime), owns their data in perspectives (knowledge graphs), and chooses which protocols (languages) to speak. Think of it as a new layer in the internet stack that doesn't replace existing protocols but harmonizes them.
</Callout>
...
- Plain HTTP (`http://localhost:3001/mcp`) — no TLS needed
- Executor and MCP client run on the same machine
...
- Enforce strong authentication (see below)

**Multi-user deployments:**
- Each user gets isolated perspective space (enforced by executor)
- JWTs scope access to user-owned perspectives only
- Consider rate limiting and monitoring for production workloads

<Callout type="warning">
AD4M executors are designed to run **locally** (one per user) or on **trusted infrastructure** (user's VPS/server). For remote access, always use TLS and strong credentials. Never expose an executor with weak credentials to the public internet.
</Callout>

### Example Reverse Proxy (nginx with TLS)
...
server {
    ...
    location /graphql {
        proxy_pass http://localhost:12100/graphql;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
    }
...
:ChannelShape a sh:NodeShape ;
  ...
  sh:property [
    ...
    sh:maxCount 1 ;
    sh:minCount 1 ;  # Required for constructor
  ] ;
  sh:property [
    sh:path :description ;
    sh:datatype xsd:string ;
  ...
  sh:property [
    sh:path :members ;
    sh:class :Agent ;  # Collection of Agents
  ] ;
  sh:property [
    sh:path :messages ;
    sh:class :Message ;  # Collection of Messages
  ] .
```

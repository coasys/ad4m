...
for (const o of overlays) {
  // o.base: the instance, o.kind: "create" | "update",
...
`interpretationOverlays()` always reads from the executor, so the list is never
stale. When several callers ask at the same time (for example, many components
...
one. Nothing is cached, so there is no cache to invalidate.

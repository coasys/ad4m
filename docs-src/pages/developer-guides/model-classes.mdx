...
- **Rows** have the shape of a `LinkExpression` (`LinkRow` in the SDK), so you can verify `proof` yourself. A link stored without a proof arrives with an empty `key` and `signature`. Treat that as unverifiable, not as valid. Links whose signature did not verify are left out unless the query sets `includeUnverified` (see [Unverified Links](#unverified-links)). Rows are sorted oldest first, and ties have a fixed order, so every replica returns the same array.

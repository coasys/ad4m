...
Getters are evaluated server-side in Rust via `evaluate_getters_batch()`, so they execute efficiently without round-trips to JavaScript.
...
```typescript
// Get all (queries execute server-side in Rust via SPARQL)
...
`Ad4mModel` uses the **Rust-side SPARQL query engine** for all query operations. Queries are compiled to SPARQL, executed server-side in Oxigraph, and results are hydrated back into model instances — all without round-trips to JavaScript.

```typescript
// All queries execute server-side via SPARQL
const recipes = await Recipe.findAll(perspective, { where: { rating: { gt: 4 } } });

// Prolog is still available for backward compatibility

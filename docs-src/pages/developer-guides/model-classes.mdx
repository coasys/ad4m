...
> **Current limit:** this filters the links that *hydrate* an instance. It does not yet filter the links that decide *which* instances come back. `where` conditions pushed into SPARQL, the class's flags, `count`/`totalCount`, `$` projection counts and the sort order behind `limit`/`offset` all still match unverified links. The consequences:
>
> - **Re-typing.** A forged flag link can make an existing node show up as an instance of a class it never belonged to. It comes back with only its genuine links, but it comes back. A node with no verified links at all cannot be conjured this way.
> - **Selection by a forged value.** `where: { name: "x" }` can return an instance that hydrates with a different, verified `name`.
> - **Silent omission.** A forged link can reorder a page and push a genuine instance past `limit`. Nothing in the result shows that a row is missing.
>

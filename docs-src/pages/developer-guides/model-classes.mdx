...
By default, this instance subgraph lives in the perspective's **single default RDF graph**, interleaved with every other instance. That is a logical subgraph (a set of connected nodes), not a physically partitioned one.

### Named-Graph Partitioning (Opt-In)

Adding `graph: true` to `@Model` roots **each instance in its own named graph**, physically partitioning the perspective's store:
...
Every write for an instance is automatically threaded into the graph identified by `ad4m://graph/<instanceId>`, deleting an instance becomes an O(1) graph drop instead of a per-link teardown, and queries can be scoped to a single instance's graph so the engine skips every other partition. This is worth it when a perspective holds many independent collections (channels, boards, documents) that are created, queried, and deleted as units.

Named-graph partitioning is fully covered — including link-level APIs, scoped SPARQL, subscriptions, and when to reach for it — in the [Named Graphs guide](/developer-guides/named-graphs).

...
> **Current limit:** this filters the links that *hydrate* an instance. It does not yet filter the links that *select* instances: `where` conditions pushed into SPARQL, the class's flags, `count`/`totalCount` and `$` projection counts still match unverified links. So `where: { name: "x" }` can return an instance that hydrates with a different, verified `name`. Tracked in [#1120](https://github.com/coasys/ad4m/issues/1120).

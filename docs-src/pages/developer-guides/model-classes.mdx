...
Use `linkStatus: "shared"` when the result is shown to another user, so nothing executor-private leaks into it. Under `"shared"`, a `local: true` property always comes back empty. Included relations inherit the setting unless their own sub-query sets `linkStatus`.

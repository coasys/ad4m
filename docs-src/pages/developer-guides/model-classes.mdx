...
This matters wherever `author` works as an authority check, such as role gating. Any peer can add a link to an instance someone else created. If the check only looked at who created the instance, a peer could add `agent -> themselves` to an admin's role instance and pass it.

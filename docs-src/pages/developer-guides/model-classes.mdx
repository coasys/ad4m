  ...
  <Tab>
    ...
    <recipe://Recipe> a sh:NodeShape ;
      ...
      sh:property [
        ...
        sh:datatype xsd:string ;
        # No maxCount = collection (multiple values)
      ] .
    ```
  </Tab>
</Tabs>
...
| `sh:PropertyShape` | Property or Collection definition |
| `sh:maxCount 1` | Scalar property (single value, generates `set_` tool) |
| No `sh:maxCount` or `> 1` | Collection (generates `add_`/`remove_` tools) |
...
| `ad4m://writable` | Property supports updates |

...
<Callout type="info" emoji="ℹ️">
  This is the read surface. Writing transitions — proposing one, voting on
  it, and firing it once a `ConsensusRule` is met — lands with the consensus
  engine and is not part of this API yet. Until then `FlowTransitionProposal`
  records only exist where a client wrote one directly.
</Callout>

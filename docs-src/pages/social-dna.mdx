...
<Callout type="info" emoji="ℹ️">
  That is the read surface. The consensus engine has since landed, so **voting
  on a proposal and firing it once a `ConsensusRule` is met now work**:
  `instance.acceptProposal(p)` adds your signature and returns whatever fired,
  `instance.rejectProposal(p)` withdraws the links you signed. Proposals are
  minted by the engine's evaluator pass after an extraction run — a client-side
  `proposeTransition`, and flow subscriptions, are still absent. See
  [Flows](/flows) for the on-graph objects, the accept/reject semantics
  (reject is not a cancel), and a worked end-to-end example.
</Callout>

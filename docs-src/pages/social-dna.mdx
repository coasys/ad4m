...
<Callout type="info" emoji="ℹ️">
  ...
  [`planning/flow-interpretation-hints-design.md`](https://github.com/coasys/ad4m/blob/dev/planning/flow-interpretation-hints-design.md)
...
// Which flows accept this expression as input?
const availableFlows = await perspective.availableFlows("task://sprint-3/T-42");
// Returns: ["Delivery"]  (or ["any"] on legacy untyped flows)

// Load the flow shape.
const flow = await perspective.getFlow("Delivery");

// Mint a fresh FlowInstance in the flow's initial state (states[0]).
const instance = await FlowInstance.start(perspective, flow, "task://sprint-3/T-42");

// Read every active FlowInstance for a flow.
const instances = await FlowInstance.findAll(perspective, "Delivery");

// Propose a state transition (any agent — including an LLM via the harness).
await instance.proposeTransition({
  toState: "InProgress",
  reason: "Alice picked it up in the standup."
});

// Aggregate the votes standing on this instance right now.
const tally = await instance.aggregateVotes({ n: 2 });

// If the tally meets the consensus rule, fire it — advances currentState on-graph.
await instance.fireIfConsensus({ n: 2 });

// Report-only variant for UIs / previews (no writes).
const preview = await instance.selectFireCandidate({ n: 2 });
```

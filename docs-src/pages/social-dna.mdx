...
// so the interpretationHint the LLM sees is right there. The value is the
// engine's cache (`ad4m://flow/current_state`), a local link of your own:
// every read of FlowInstance rows derives the state for the reading user
// and refreshes that user's cache first.

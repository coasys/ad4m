...
This section introduces flows as social DNA. The [Flows](/flows) page covers the engine: the on-graph objects, how state is derived rather than stored, the accept/reject write surface, and a worked end-to-end example.
...
// so the interpretationHint the LLM sees is right there.
...
<Callout type="info" emoji="ℹ️">
  The above is the read surface. The consensus engine has since landed, so
  voting on a proposal and firing it once a `ConsensusRule` is met are now
  supported: `instance.acceptProposal(p)` adds the caller's signature and
  returns whatever fired, and `instance.rejectProposal(p)` withdraws the links
  that caller signed. Proposals are minted by the engine's evaluator pass after
  an extraction run. A client-side `proposeTransition` and flow subscriptions
  are not yet implemented. See [Flows](/flows) for the on-graph objects, the
  accept/reject semantics, and a worked end-to-end example.
</Callout>

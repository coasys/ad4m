...
Flows describe how a group advances an expression through a state machine, with each transition backed by an on-graph `FlowTransitionProposal` and (optionally) a `ConsensusRule` gating when a proposal fires. Each `FlowState` carries an `interpretationHint` — a short English sentence the interpretation engine surfaces to the LLM so agent-authored proposals stay coherent with the flow's intent.

This section introduces flows as social DNA. The [Flows](/flows) page covers the engine: the on-graph objects, how state is derived rather than stored, `proposeTransition` / accept / reject, role gates including `producedByFlow`, and a worked end-to-end example.

<Callout type="info" emoji="ℹ️">
  Flow state is tracked on-graph via `FlowInstance` (per-subject state) and
  `FlowTransitionProposal` (per-vote), not by the ad-hoc `todo://state`-style
  links used in earlier iterations. See
  [`docs/flow-interpretation-hints-design.md`](https://github.com/coasys/ad4m/blob/dev/docs/flow-interpretation-hints-design.md)
  for the full v5 design.
</Callout>

Here's a Delivery flow expressed with the `SHACLFlow` class — the shape a group would share as social DNA:
...
deliveryFlow.addState({ name: "Identified", value: 0, interpretationHint: "The Task has been named but not yet scoped." });
deliveryFlow.addState({ name: "Scoped",     value: 1, interpretationHint: "The Task has an acceptance criterion." });
deliveryFlow.addState({ name: "InProgress", value: 2, interpretationHint: "Someone is actively working on it." });
deliveryFlow.addState({ name: "Review",     value: 3, interpretationHint: "The Task is ready for peer review." });
deliveryFlow.addState({ name: "Done",       value: 4, interpretationHint: "The Task is accepted and complete." });
...
// Optional: require n distinct DIDs to co-sign a proposal before it fires.
...
// Which flows accept this expression as input? Returns flow names whose
// `inputTypes` match the expression's registered subject classes, plus any
// flow declaring no `inputTypes` or the "any" wildcard.
const availableFlows = await perspective.availableFlows("task://sprint-3/T-42");
// Returns: ["Delivery"]

// Mint a fresh FlowInstance in the flow's initial state (states[0] — the
// lowest-valued state). Takes the flow *name*; the shape is loaded for you.
// The executor writes the instance and its initial state
// (`perspective.startFlowInstance(flowUri, subject)` underneath).
const instance = await FlowInstance.start(
  perspective,
  "Delivery",
  "task://sprint-3/T-42",
);

// Read every active FlowInstance for a flow. The filter also accepts
// `{ flowName?, subject? }` — pass `subject` to ask "what is running on
// this expression?".
const instances = await FlowInstance.findAll(perspective, "Delivery");

// Where is it now? `currentState` resolves to the FlowState on the shape,
// so the interpretationHint the LLM sees is right there. The executor's flow
// engine maintains this value (`ad4m://flow/current_state`); clients cannot
// write that predicate, and a direct `add` / `update` of it is refused.
console.log(instance.currentStateName);            // "Identified"
console.log(instance.currentState.interpretationHint);

// Which actions are legal from here? Empty on a terminal state.
for (const t of instance.availableTransitions) {
  console.log(t.actionName, "→", t.toState);       // "Scope" → "Scoped"
}

// Proposals standing against this instance.
const proposals = await instance.proposals();
```

<Callout type="info" emoji="ℹ️">
  That is the read surface. The write surface is on the same wrapper:
  `instance.proposeTransition(toState, rationale?)` is the button click
  (mint or co-sign), `instance.acceptProposal(p)` adds a signature to a
  listed proposal, `instance.rejectProposal(p)` withdraws the links you
  signed. Reject is not a cancel — retracting a settling vote moves the
  flow back. See [Flows](/flows) for `FlowProposeResult`, role gates, and
  a worked end-to-end example. The executable twin is
  `tests/js/tests/model/flow-task-handover.test.ts`.
</Callout>

### Adding Flows to a Perspective

```typescript
// Recommended: type-safe helper takes the SHACLFlow instance directly.
await perspective.addFlow("Delivery", deliveryFlow);

const flows = await perspective.sdnaFlows();
console.log(flows); // Should include "Delivery"
```

## Query Engine: SPARQL

AD4M provides a high-performance SPARQL query engine for working with Social DNA and perspective data, powered by Oxigraph and executing entirely server-side in Rust.

### SPARQL: Fast Graph Queries

SPARQL is the standard query language for RDF data and is the primary query engine in AD4M:
- Direct queries against the perspective's RDF graph
- Filtering, sorting, and aggregation
- Multi-hop graph traversal via pattern matching
- Server-side execution for optimal performance
...
**Note**: `Ad4mModel` uses the SPARQL engine internally for `findAll()` and query builder operations. See the [SPARQL Queries Guide](/developer-guides/sparql-queries) for comprehensive examples.

### The Prolog Engine (Backward Compatibility)

<Callout type="warning" emoji="⚠️">
  The Prolog engine is maintained for backward compatibility. For new development,
  use SPARQL queries or the `Ad4mModel` system (which uses SPARQL internally).
</Callout>
...
**When to use which?**
- **Use SPARQL** for: Fast queries, graph traversal, analytics (most use cases)
...
```typescript
// Option 1: Use Ad4mModel (recommended — queries execute server-side via SPARQL)
const activeTodos = await Todo.findAll(perspective, {
  where: { state: "active" }
});

// Option 2: Direct SPARQL query
const todos = await perspective.querySparql(`
  SELECT ?todo WHERE {
    ?link a rdf:Statement ;
          rdf:subject ?todo ;
          rdf:predicate <todo://state> ;
...
// Option 3: Prolog (for backward compatibility)

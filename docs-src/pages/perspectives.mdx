...
Every Perspective includes a high-performance **SPARQL query engine** powered by Oxigraph, executing entirely server-side in Rust. SPARQL is the recommended query method for most use cases:

```typescript
// Direct SPARQL query - fast and powerful
...
**Security Note**: `querySparql()` only allows read-only SELECT queries. Use `perspective.add()` and `perspective.remove()` to modify links.

For comprehensive examples and graph traversal patterns, see the [SPARQL Queries Guide](/developer-guides/sparql-queries).
...
**Recommendation**: Use SPARQL for most queries, and reserve Prolog for complex logic programming or custom SDNA rules.
...
Each link comes back with the exact `target` it was signed over. The store
indexes `literal:string:`, `literal:number:`, `literal:boolean:` and
`literal:json:` targets as typed values, so a query finds a value however it
was percent-encoded, but `get()` still returns the target as written, including
literals not built with `Literal.from(value).toUrl()` (for example
`literal:string:Hello world` with a raw space). That keeps the `proof` valid
when a read-back link is passed on, for example when the Perspective is later
published as a Neighbourhood.

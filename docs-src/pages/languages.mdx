...
> **⚠️ New Language Interface (v1.0):** AD4M is migrating from the
> `create(context)` factory pattern described below to a **flat export
> model** where each Language is a module of named exports (`init`,
> `teardown`, `perspectiveCommit`, `expressionGet`, etc.) with no
> factory function and no context parameter. The runtime provides
> services via imports (`agentDid()`, `holochainCall()`, `storageGet()`,
> etc.) rather than a context object. See
> [`docs/language-interface-spec.md`](https://github.com/coasys/ad4m/blob/dev/docs/language-interface-spec.md)
> for the full spec and the `@coasys/ad4m-ldk` package for the
> authoring SDK. The legacy `create()` path continues to work during
> the transition.
